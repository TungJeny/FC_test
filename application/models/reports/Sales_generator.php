...
class Sales_generator extends Report
{
    ...
    public function getDataColumns()
    {
        ...
        $return['summary'][] = array(
            'data' => lang('reports_sale_id'),
            'align' => 'left'
        );
        
        if ($location_count > 1) {
            $return['summary'][] = array(
                'data' => lang('common_location'),
                'align' => 'left'
            );
        }
        ...
        $return['summary'][] = array(
            'data' => lang('reports_register'),
            'align' => 'left'
        );
        $return['summary'][] = array(
            'data' => lang('common_items_purchased'),
            'align' => 'left'
        );
        $return['summary'][] = array(
        ...
        $return['summary'][] = array(
            'data' => lang('reports_sold_to'),
            'align' => 'left'
        );
        $return['summary'][] = array(
            'data' => lang('reports_subtotal'),
            'align' => 'right'
        );
        $return['summary'][] = array(
        ...
        $return['summary'][] = array(
            'data' => lang('common_tax'),
            'align' => 'right'
        );
        
        if ($this->Employee->has_module_action_permission('reports', 'show_profit', $this->Employee->get_logged_in_employee_info()->person_id)) {
            $return['summary'][] = array(
                'data' => lang('common_profit'),
                'align' => 'right'
            );
        }
        $return['summary'][] = array(
        ...
        $return['summary'][] = array(
            'data' => lang('reports_comments'),
            'align' => 'right'
        );
        $return['summary'][] = array(
            'data' => lang('common_discount_reason'),
            'align' => 'right'
        );
        
        $return['details'] = array();
        $return['details'][] = array(
            'data' => lang('common_item_number'),
            'align' => 'left'
        );
        $return['details'][] = array(
            'data' => lang('common_product_id'),
            'align' => 'left'
        );
        $return['details'][] = array(
            'data' => lang('reports_name'),
            'align' => 'left'
        );
        $return['details'][] = array(
            'data' => lang('reports_category'),
            'align' => 'left'
        );
        $return['details'][] = array(
            'data' => lang('common_size'),
            'align' => 'left'
        );
        $return['details'][] = array(
            'data' => lang('reports_serial_number'),
            'align' => 'left'
        );
        $return['details'][] = array(
        ...
        $return['details'][] = array(
            'data' => lang('reports_current_selling_price'),
            'align' => 'left'
        );
        $return['details'][] = array(
            'data' => lang('reports_quantity_purchased'),
            'align' => 'left'
        );
        $return['details'][] = array(
            'data' => lang('reports_subtotal'),
        ...
        if ($this->Employee->has_module_action_permission('reports', 'show_cost_price', $this->Employee->get_logged_in_employee_info()->person_id)) {
            $return['details'][] = array(
                'data' => lang('common_cost_price'),
                'align' => 'right'
            );
        }
        
        $return['details'][] = array(
            'data' => lang('common_discount'),
            'align' => 'right'
        );
    ...
    public function getData()
    {
        ...
        if ($this->params['matched_items_only']) {
            ...
            $this->db->select('customer_data.account_number as account_number, locations.name as location_name, sales.sale_id, sale_time, registers.name as register_name, date(sale_time) as sale_date, ' . $this->db->dbprefix('sales') . '.total_quantity_purchased as items_purchased, CONCAT(sold_by_employee.first_name," ",sold_by_employee.last_name) as sold_by_employee, CONCAT(employee.first_name," ",employee.last_name) as employee_name, CONCAT(customer.first_name," ",customer.last_name) as customer_name, ' . $this->db->dbprefix('sales') . '.subtotal as subtotal, ' . $this->db->dbprefix('sales') . '.total as total, ' . $this->db->dbprefix('sales') . '.tax as tax, ' . $this->db->dbprefix('sales') . '.profit as profit, sales.payment_type, sales.comment, sales.discount_reason', false);
            $this->db->from('sales');
    ...
    public function getSummaryData()
    {
        if ($this->params['matched_items_only']) {
            ...
            $this->db->select('sales.sale_id, (' . $this->db->dbprefix('sales') . '.subtotal) as subtotal, (' . $this->db->dbprefix('sales') . '.total) as total, (' . $this->db->dbprefix('sales') . '.tax) as tax, (' . $this->db->dbprefix('sales') . '.profit) as profit,(total_quantity_purchased) as items_purchased', false);
            $this->db->from('sales');
            $this->db->join('sales_items', 'sales_items.sale_id = sales.sale_id', 'left');
            $this->db->join('sales_item_kits', 'sales_item_kits.sale_id = sales.sale_id', 'left');
            $this->db->join('items_tags', 'sales_items.item_id = items_tags.item_id', 'left');
    ...
    private function _searchSalesQueryParams()
    {
        ...
        if ($this->params['values'][0]['f'] != 0) {
            ...
            foreach ($this->params['values'] as $w => $d) {
                ...
                if (count($d['i']) > 1) {
                    if ($d['o'] == 1) {
                        $ops = $this->params['ops'][5];
                    }
                    if ($d['o'] == 2) {
                        $ops = $this->params['ops'][6];
                    }
                }
                ...
                if ($d['f'] == 6 && $d['o'] == 10) {
                    ...
                    $this->db->or_where('items_tags.item_id' . ' ' . str_replace("xx", join("', '", $d['i']), $ops));
                    $this->db->or_where('item_kits_tags.item_kit_id' . ' ' . str_replace("xx", join("', '", $d['i']), $ops));
                    $this->db->group_end();
                } elseif ($d['f'] == 15) {
                    $this->db->$matchTypeGroup();
