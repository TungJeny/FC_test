...
class Inventory_summary extends Report
{
    ...
    public function getDataColumns()
    {
        ...
        $columns[] = array(
            'data' => lang('common_item_id'),
            'align' => 'left'
        );
        $columns[] = array(
            'data' => lang('reports_item_name'),
            'align' => 'left'
        );
        $columns[] = array(
        ...
        $columns[] = array(
            'data' => lang('common_unit_price'),
            'align' => 'left'
        );
        $columns[] = array(
            'data' => lang('common_count'),
            'align' => 'left'
        );
        $columns[] = array(
            'data' => lang('reports_pending_inventory'),
            'align' => 'left'
        );
        $columns[] = array(
            'data' => lang('reports_reorder_level'),
            'align' => 'left'
        );
    ...
    public function getData()
    {
        ...
        if ($this->params['inventory_date'] == date('Y-m-d') . ' 23:59:59') {
            ...
            $sum_query = '(SELECT SUM(trans_inventory) FROM ' . $this->db->dbprefix('inventory') . ' WHERE 
				' . $this->db->dbprefix('inventory') . '.trans_date < "' . $this->params['inventory_date'] . '" and ' . $this->db->dbprefix('inventory') . '.trans_items=' . $this->db->dbprefix('items') . '.item_id and ' . $this->db->dbprefix('inventory') . '.location_id =location_id_outer
			)';
        }
        
        $this->db->select('location_id as location_id_outer, items.item_id, items.name, categories.id as category_id,categories.name as category,location, company_name, item_number,size, product_id, 
    ...
    public function getSummaryData()
    {
        ...
        if ($this->params['inventory_date'] == date('Y-m-d') . ' 23:59:59') {
            ...
            $full_sum_query = '(SELECT SUM(trans_inventory) FROM ' . $this->db->dbprefix('inventory') . ' WHERE 
				' . $this->db->dbprefix('inventory') . '.trans_date < "' . $this->params['inventory_date'] . '" and ' . $this->db->dbprefix('inventory') . '.location_id =location_id_outer
			)';
        ...
        $this->db->select('location_id as location_id_outer,' . $full_sum_query . ' as total_items_in_inventory, sum(IFNULL(' . $this->db->dbprefix('location_items') . '.cost_price, ' . $this->db->dbprefix('items') . '.cost_price) * ' . $quantity_query . ') as inventory_total,
        ...
        unset($result['location_id_outer']);
        return $result;
    }
}
?>