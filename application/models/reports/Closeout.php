...
class Closeout extends Report
{
    ...
    public function getData()
    {
        ...
        if (! $this->params['export_excel']) {
            $return[] = array(
                anchor('reports/closeout/' . $yesterday . '/' . $yesterday . ' 23:59:59', '<span class="glyphicon glyphicon-backward"></span> ' . lang('reports_previous_day'), array(
                    'class' => 'pull-left'
                )),
                anchor('reports/closeout/' . $tomorrow . '/' . $tomorrow . ' 23:59:59', lang('reports_next_day') . ' <span class="glyphicon glyphicon-forward"></span>', array(
                    'class' => 'pull-right'
                ))
            );
        }
        ...
        $return[] = array(
            '<h1>' . lang('reports_all_transactions') . '</h1>',
            '--'
        );
        
        $return[] = array(
            lang('reports_total') . ' (' . lang('common_without_tax') . ')',
            isset($sales_row['total']) ? to_currency($sales_row['total'] - $sales_row['tax']) : 0
        );
        $return[] = array(
            lang('reports_total') . ' (' . lang('reports_items_with_tax') . ')',
            isset($sales_row['total']) ? to_currency($sales_row['total']) : 0
        );
        if ($this->Employee->has_module_action_permission('reports', 'show_profit', $this->Employee->get_logged_in_employee_info()->person_id)) {
            $return[] = array(
                lang('reports_profit'),
                isset($sales_row['profit']) ? to_currency($sales_row['profit']) : 0
            );
        }
        ...
        foreach ($category_sales as $category_sale_row) {
            $return[] = array(
                $this->Category->get_full_path($category_sale_row['category_id']),
                to_currency($category_sale_row['total'])
            );
        }
        $return[] = array(
            ' ',
        ...
        $return[] = array(
            lang('reports_number_of_transactions'),
            to_quantity($number_of_sales_transactions)
        );
        $return[] = array(
            lang('reports_average_ticket_size'),
            to_currency($average_ticket_size)
        );
        
        $return[] = array(
            lang('common_items_sold'),
            isset($sales_row['quantity']) ? to_quantity($sales_row['quantity']) : 0
        );
        ...
        $return[] = array(
            lang('common_tax'),
            isset($sales_row['tax']) ? to_currency($sales_row['tax']) : 0
        );
        
        $this->load->model('reports/Summary_taxes');
        
        $this->Summary_taxes->setParams(array(
        ...
        foreach ($taxes as $tax_row) {
            if ($tax_row['name'] != lang('reports_non_taxable')) {
                $return[] = array(
                    $tax_row['name'],
                    lang('common_tax') . ': ' . to_currency($tax_row['tax']) . '<br />' . lang('reports_subtotal') . ': ' . to_currency($tax_row['subtotal']) . '<br />' . lang('reports_total') . ': ' . to_currency($tax_row['total'])
                );
            }
        }
        ...
        if (isset($taxes[lang('reports_non_taxable')])) {
            $return[] = array(
                lang('reports_non_taxable'),
                to_currency($taxes[lang('reports_non_taxable')]['total'])
            );
        }
        ...
        foreach ($payment_data as $payment_row) {
            $return[] = array(
                ...
                to_currency($payment_row['payment_amount'])
            );
        }
        ...
        $return[] = array(
            '<h1>' . lang('reports_sales') . '</h1>',
            '--'
        );
        $return[] = array(
            lang('reports_total_sales') . ' (' . lang('common_without_tax') . ')',
            isset($sales_row['total']) ? to_currency($sales_row['total'] - $sales_row['tax']) : 0
        );
        $return[] = array(
            lang('reports_total_sales') . ' (' . lang('reports_items_with_tax') . ')',
            isset($sales_row['total']) ? to_currency($sales_row['total']) : 0
        );
        if ($this->Employee->has_module_action_permission('reports', 'show_profit', $this->Employee->get_logged_in_employee_info()->person_id)) {
            $return[] = array(
        ...
        $return[] = array(
            '<h1>' . lang('reports_returns') . '</h1>',
            '--'
        );
        $return[] = array(
            lang('reports_total') . ' (' . lang('common_without_tax') . ')',
            isset($sales_row['total']) ? to_currency(abs($sales_row['total'] - $sales_row['tax'])) : 0
        );
        $return[] = array(
            lang('reports_total') . ' (' . lang('reports_items_with_tax') . ')',
            isset($sales_row['total']) ? to_currency(abs($sales_row['total'])) : 0
        );
        
        $return[] = array(
            lang('reports_total_returned'),
            isset($sales_row['total']) ? to_currency(abs($sales_row['total'])) : 0
        );
        ...
        foreach ($category_returns as $category_sale_row) {
            $return[] = array(
                $this->Category->get_full_path($category_sale_row['category_id']),
                to_currency(abs($category_sale_row['total']))
            );
        }
        ...
        $return[] = array(
            lang('reports_number_of_transactions'),
            to_quantity($number_of_returned_transactions)
        );
        $return[] = array(
            lang('reports_items_returned'),
            isset($sales_row['quantity']) ? to_quantity(abs($sales_row['quantity'])) : 0
        );
        $return[] = array(
            lang('common_tax'),
            isset($sales_row['tax']) ? to_currency(abs($sales_row['tax'])) : 0
        );
        ...
        foreach ($payment_data as $payment_row) {
            $return[] = array(
                $payment_row['payment_type'],
                to_currency(abs($payment_row['payment_amount']))
            );
        }
        ...
        $return[] = array(
            lang('reports_number_of_discounts'),
            isset($sales_row['quantity']) ? to_quantity(abs($sales_row['quantity'])) : 0
        );
        
        $return[] = array(
            '<h1>' . lang('common_inv') . '</h1>',
            '--'
        );
        $this->load->model('reports/Inventory_summary');
        $model_inv_sum = $this->Inventory_summary;
        $model_inv_sum->setParams(array(
            'inventory_date' => $this->params['end_date'],
            'supplier' => - 1,
            'category_id' => - 1,
        ...
        $return[] = array(
            lang('reports_total_items_in_inventory'),
            to_quantity($summary_data['total_items_in_inventory'])
        );
        $return[] = array(
            lang('reports_inventory_total'),
            to_currency($summary_data['inventory_total'])
        );
        ...
        if ($this->config->item('enable_customer_loyalty_system') && $this->config->item('loyalty_option') == 'advanced') {
            ...
            $return[] = array(
                '<h1>' . lang('reports_loyalty') . '</h1>',
                '--'
            );
            $return[] = array(
                lang('reports_points_used'),
                to_currency_no_money($points['points_used'])
            );
            $return[] = array(
                lang('reports_points_earned'),
                to_currency_no_money($points['points_gained'])
            );
        }
        if ($this->config->item('customers_store_accounts')) {
            ...
            $return[] = array(
                '<h1>' . lang('reports_store_account') . '</h1>',
                '--'
            );
            ...
            $return[] = array(
                lang('reports_debits'),
                to_currency($store_account_info['debits'])
            );
            $return[] = array(
                lang('reports_credits'),
                to_currency(abs($store_account_info['credits']))
            );
            $return[] = array(
                lang('reports_total_balance_of_all_store_accounts'),
                to_currency($store_account_info['total_balance_of_all_store_accounts'])
            );
        }
        ...
        if ($this->config->item('suppliers_store_accounts')) {
            ...
            $return[] = array(
                '<h1>' . lang('reports_store_account_suppliers') . '</h1>',
                '--'
            );
        ...
        $return[] = array(
            '<h1>' . lang('reports_receivings') . '</h1>',
            '--'
        );
        $return[] = array(
            lang('reports_total_receivings') . ' (' . lang('common_without_tax') . ')',
            isset($recvs_row['total']) ? to_currency($recvs_row['total'] - $recvs_row['tax']) : 0
        );
        $return[] = array(
            lang('reports_total_receivings') . ' (' . lang('reports_items_with_tax') . ')',
            isset($recvs_row['total']) ? to_currency($recvs_row['total']) : 0
        );
        $return[] = array(
            ' ',
            ' '
        ...
        // print_r($category_recvs ); exit;
        
        foreach ($category_recvs as $category_recv_row) {
            $return[] = array(
                $this->Category->get_full_path($category_recv_row['category_id']),
                to_currency($category_recv_row['total'])
            );
        }
        $return[] = array(
            ' ',
        ...
        $return[] = array(
            lang('reports_number_of_transactions'),
            to_quantity($number_of_recevings_transactions)
        );
        $return[] = array(
            lang('reports_average_ticket_size'),
            to_currency($average_ticket_size)
        ...
        $return[] = array(
            lang('reports_items_recved'),
            isset($recvs_row['quantity']) ? to_quantity($recvs_row['quantity']) : 0
        );
        $return[] = array(
            ' ',
            ' '
        ...
        $return[] = array(
            lang('common_tax'),
            isset($recvs_row['tax']) ? to_currency($recvs_row['tax']) : 0
        );
        
        $taxes_data = array();
        $this->load->model('reports/Summary_taxes_receivings');
        
        $this->Summary_taxes_receivings->setParams(array(
            'start_date' => $this->params['start_date'],
            'end_date' => $this->params['end_date'],
            'sale_type' => 'sales'
        ));
        ...
        $return[] = array(
            '<h1>' . lang('common_expenses') . '</h1>',
            '--'
        );
        $return[] = array(
            lang('reports_total_expenses'),
            to_currency($total)
        );
        ...
        foreach ($category_expenses as $category_sale_row) {
            $return[] = array(
                $this->Category->get_full_path($category_sale_row['category_id']),
                to_currency($category_sale_row['amount'])
            );
        }
        ...
        if ($this->config->item('track_cash')) {
            ...
            foreach ($cash_logging as $cash_logging_row) {
                ...
                $return[] = array(
                    '<h2>' . $cash_logging_row['register_name'] . ' (' . $cash_logging_row['location_name'] . ')</h2>' . date(get_date_format() . ' ' . get_time_format(), strtotime($cash_logging_row['shift_start'])) . ' - ' . date(get_date_format() . ' ' . get_time_format(), strtotime($cash_logging_row['shift_end'])),
                    $data
                );
            }
        }
