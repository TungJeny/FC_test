...
class Summary_customers extends Report
{
    ...
    public function getDataColumns()
    {
        ...
        $columns[] = array(
            'data' => lang('common_person_id'),
            'align' => 'left'
        );
        $columns[] = array(
            'data' => lang('reports_customer'),
            'align' => 'left'
        );
        $columns[] = array(
            'data' => lang('common_phone_number'),
            'align' => 'left'
        );
        $columns[] = array(
        ...
        if ($this->config->item('enable_customer_loyalty_system') && $this->config->item('loyalty_option') == 'advanced') {
            $columns[] = array(
                'data' => lang('reports_points_used'),
                'align' => 'left'
            );
            $columns[] = array(
                'data' => lang('reports_points_earned'),
                'align' => 'left'
            );
        } elseif ($this->config->item('enable_customer_loyalty_system') && $this->config->item('loyalty_option') == 'simple') {
            $columns[] = array(
                'data' => lang('common_sales_until_discount'),
                'align' => 'left'
            );
        }
    ...
    {
        $this->db->select('customers.current_sales_for_discount, customer_id, CONCAT(first_name, " ",last_name) as customer, customers.person_id as person_id, people.phone_number, sum(subtotal) as subtotal, sum(total) as total, sum(tax) as tax,sum(profit) as profit', false);
        $this->db->from('sales');
