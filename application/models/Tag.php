...
class Tag extends CI_Model
{
    ...
    {
        $this->db->where('id', $tag_id);
        return $this->db->update('tags', array(
            'deleted' => 1,
            'name' => NULL
        ));
    }
    ...
    function save_tags_for_item($item_id, $tags)
    {
        ...
        $this->db->delete('items_tags', array(
            'item_id' => $item_id
        ));
        ...
        foreach ($tags as $tag) {
            if ($tag != '') {
                ...
                {
                    $this->db->insert('items_tags', array(
                        'item_id' => $item_id,
                        'tag_id' => $tag
                    ));
    ...
    {
        // Remove current tags for item
        $this->db->delete('item_kits_tags', array(
        ...
        foreach ($tags as $tag) {
            if ($tag != '') {
                ...
                if (is_numeric($tag) && $this->tag_id_exists($tag)) // Numeric Tag ID
                {
                    $this->db->insert('item_kits_tags', array(
                        'item_kit_id' => $item_kit_id,
                        'tag_id' => $tag
                    ));
                } elseif ($this->tag_name_exists($tag)) // Named tag
