...
class Item_location extends CI_Model
{
    ...
    function save_quantity($quantity, $item_id, $location_id = false)
    {
        ...
        $sql = 'INSERT INTO ' . $this->db->dbprefix('location_items') . ' (quantity, item_id, location_id)' . ' VALUES (?, ?, ?)' . ' ON DUPLICATE KEY UPDATE quantity = ?';
        ...
        if ($select_inventory) {
            if ($this->Item->is_empty_search()) {
                if ($quantity_change) {
                    ...
                    $items = $this->Item->get_all($this->Item->count_all())
                        ->result_object();
    ...
    function get_info($item_id, $location = false, $can_cache = false)
    {
        ...
        if ($query->num_rows() == 1) {
            ...
            $row->is_overwritten = ($row->cost_price !== NULL || $row->unit_price !== NULL || $row->promo_price !== NULL || $this->is_tier_overwritten($item_id, $location));
    ...
    function get_qty_inventory_in_month($item_id, $month = '')
    {
        $this->db->from('location_items');
        $this->db->where('item_id', $item_id);
        $query = $this->db->get();
        
        $records = !empty($query) ? $query->result_array() : [];
        $total_qty = 0;
