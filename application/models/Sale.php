...
class Sale extends CI_Model
{
                ...
                foreach ($payment_rows as $payment_row) {
                    ...
                    if (! isset($foreign_language_to_cur_language[$payment_row['payment_type']]) || ! isset($payment_data[$foreign_language_to_cur_language[$payment_row['payment_type']]])) {
                        ...
                        if (strpos($payment_row['payment_type'], ':') !== FALSE && ! isset($foreign_language_to_cur_language[$payment_row['payment_type']])) {
                            ...
                            $payment_data[$foreign_language_to_cur_language[$payment_row['payment_type']]] = array(
                                'payment_type' => $foreign_language_to_cur_language[$payment_row['payment_type']],
                                'payment_amount' => 0
                            );
                            ...
                            $payment_data[$payment_row['payment_type']] = array(
                    ...
                    if (($total_sale_balance != 0 || ($sales_totals[$sale_id] >= 0 && $payment_row['payment_amount'] < 0) || ($sales_totals[$sale_id] < 0 && $payment_row['payment_amount'] >= 0)) && $exists) {
                ...
                foreach ($payment_rows as $payment_row) {
                    ...
                    if (! isset($foreign_language_to_cur_language[$payment_row['payment_type']]) || ! isset($payment_data[$sale_id][$foreign_language_to_cur_language[$payment_row['payment_type']]])) {
                        ...
                        if (strpos($payment_row['payment_type'], ':') !== FALSE && ! isset($foreign_language_to_cur_language[$payment_row['payment_type']])) {
                            list ($giftcard_translation, $giftcard_number) = explode(":", $payment_row['payment_type']);
                            $foreign_language_to_cur_language[$payment_row['payment_type']] = $foreign_language_to_cur_language[$giftcard_translation] . ':' . $giftcard_number;
                            $payment_data[$sale_id][$foreign_language_to_cur_language[$payment_row['payment_type']]] = array(
                                'sale_id' => $sale_id,
                                'payment_type' => $foreign_language_to_cur_language[$payment_row['payment_type']],
                                'payment_amount' => 0,
                                'payment_date' => $payment_row['payment_date'],
                                'sale_time' => $payment_row['sale_time']
                            );
                            ...
                            $payment_data[$sale_id][$payment_row['payment_type']] = array(
                                'sale_id' => $sale_id,
                                'payment_type' => $payment_row['payment_type'],
                                'payment_amount' => 0,
                                'payment_date' => $payment_row['payment_date'],
    ...
    function save($items, $customer_id, $employee_id, $sold_by_employee_id, $comment, $discount_reason, $show_comment_on_receipt, $payments, $sale_id = false, $suspended = 0, $change_sale_date = false, $balance = 0, $store_account_payment = 0, $rule = array())
    {
        ...
        // if(!empty($rule) && isset($rule['rule_id']) && isset($rule['spending_discount']) && $rule['spending_discount'] > 0)
        // {
        // $sales_data['rule_id']=$rule['rule_id'];
        // $sales_data['rule_discount']=$rule['spending_discount'];
        // }
        ...
        if (! empty($paid_sales)) {
            foreach (array_keys($this->sale_lib->get_paid_store_account_sales()) as $sale_id_paid) {
                $this->db->insert('store_accounts_paid_sales', array(
                    'sale_id' => $sale_id_paid,
                    'store_account_payment_sale_id' => $sale_id
                ));
            }
        }
            ...
            if (! $customer_info->disable_loyalty) {
                if ($this->config->item('loyalty_option') == 'simple') {
                    ...
                    $this->db->update('customers', array(
                        ...
                        'current_spend_for_points' => $new_current_spend_for_points
                    ));
                }
        ...
        foreach ($payments as $payment_id => $payment) {
            ...
            if ($suspended != 2) {
                if (substr($payment['payment_type'], 0, strlen(lang('common_giftcard'))) == lang('common_giftcard')) {
                    ...
                    $this->Giftcard->log_modification(array(
                        'sale_id' => $sale_id,
                        "number" => $splitpayment[1],
                        "person" => lang('common_customer'),
                        "old_value" => $cur_giftcard_value,
        ...
        foreach ($items as $line => $item) {
            isset($item['rule']['rule_discount']) && $item['rule']['rule_discount'] > 0 ? $item_rule_discount = $item['rule']['rule_discount'] : $item_rule_discount = 0;
            
            $sale_item_subtotal = $this->sale_lib->get_item_subtotal($line) - ($item_rule_discount);
            $sale_item_total = $this->sale_lib->get_item_total($line) - ($item_rule_discount);
            ...
            if (isset($item['item_id'])) {
                ...
                $sale_item_profit = $this->sale_lib->get_item_profit($line, $cost_price) - ($item_rule_discount);
                ...
                if ($suspended != 2) {
                    ...
                    if ($item['name'] == lang('common_giftcard') && ! $this->Giftcard->get_giftcard_id($item['description'])) {
                        ...
                        $this->Giftcard->log_modification(array(
                            ...
                            "person" => $employee_info->first_name . " " . $employee_info->last_name,
        ...
        if ($sales_payment->num_rows() >= 1) {
            foreach ($sales_payment->result() as $row) {
                ...
                $this->Giftcard->log_modification(array(
                    ...
                    "old_value" => $cur_giftcard_value,
                    "new_value" => $cur_giftcard_value + $value_to_add_subtract,
                    "type" => $undelete ? 'sale_undelete' : 'sale_delete'
                ));
            }
        }
    }
    ...
    function delete($sale_id, $all_data = false)
    {
        ...
        if ($all_data) {
            ...
            $this->db->delete('sales_item_kits_taxes', array(
                'sale_id' => $sale_id
            ));
            $this->db->delete('sales_item_kits', array(
                'sale_id' => $sale_id
            ));
        }
        
        $this->db->where('sale_id', $sale_id);
        return $this->db->update('sales', array(
            'deleted' => 1,
    ...
    function get_sale_items_taxes($sale_id, $line = FALSE)
    {
        ...
        $query = $this->db->query('SELECT name, percent, cumulative, item_unit_price as price, quantity_purchased as quantity, discount_percent as discount ' . 'FROM ' . $this->db->dbprefix('sales_items_taxes') . ' JOIN ' . $this->db->dbprefix('sales_items') . ' USING (sale_id, item_id, line) ' . 'WHERE ' . $this->db->dbprefix('sales_items_taxes') . ".sale_id = $sale_id" . ' ' . $item_where . ' ' . 'ORDER BY ' . $this->db->dbprefix('sales_items') . '.line,' . $this->db->dbprefix('sales_items') . '.item_id,cumulative,name,percent');
    ...
    function get_sale_item_kits_taxes($sale_id, $line = FALSE)
    {
        ...
        $query = $this->db->query('SELECT name, percent, cumulative, item_kit_unit_price as price, quantity_purchased as quantity, discount_percent as discount ' . 'FROM ' . $this->db->dbprefix('sales_item_kits_taxes') . ' JOIN ' . $this->db->dbprefix('sales_item_kits') . ' USING (sale_id, item_kit_id, line) ' . 'WHERE ' . $this->db->dbprefix('sales_item_kits_taxes') . ".sale_id = $sale_id" . ' ' . $item_kit_where . ' ' . 'ORDER BY ' . $this->db->dbprefix('sales_item_kits') . '.line,' . $this->db->dbprefix('sales_item_kits') . '.item_kit_id,cumulative,name,percent');
        return $query->result_array();
    }
    ...
    function get_customer($sale_id)
    {
        $this->db->from('sales');
        $this->db->where('sale_id', $sale_id);
        return $this->Customer->get_info($this->db->get()
            ->row()->customer_id);
    }
    ...
    function get_sales_amount_for_range($start_date, $end_date)
    {
        ...
        foreach ($data as $row) {
            $report_data[] = array(
                ...
                'sale_amount' => to_currency_no_money($row['total'])
            );
        }
    ...
    function can_void_cc_sale($sale_id)
    {
        ...
        $this->db->from('sales_payments');
        $this->db->where('sale_id', $sale_id);
        $this->db->where_in('payment_type', array(
            lang('common_credit'),
            lang('sales_partial_credit')
        ));
        ...
        foreach ($result as $row) {
            if ($processor == 'mercury' || $processor == 'heartland' || $processor = 'evo' || $processor = 'worldpay' || $processor == 'other_usb') {
    ...
    function get_payment_options()
    {
        ...
        if ($this->Location->get_info_for_key('enable_credit_card_processing')) {
            ...
            if ($this->config->item('customers_store_accounts') && $this->sale_lib->get_mode() != 'store_account_payment') {
                $payment_options = array_merge($payment_options, array(
            ...
            if (isset($cust_info) && ! $cust_info->disable_loyalty) {
                if ($this->config->item('enable_customer_loyalty_system') && $this->config->item('loyalty_option') == 'advanced' && count(explode(":", $this->config->item('spend_to_point_ratio'), 2)) == 2 && isset($cust_info) && $cust_info->points >= 1 && $this->sale_lib->get_payment_amount(lang('common_points')) <= 0) {
                    $payment_options = array_merge($payment_options, array(
                        lang('common_points') => lang('common_points')
                    ));
                }
            }
            ...
            if ($this->config->item('enable_ebt_payments')) {
                $payment_options = array_merge($payment_options, array(
                    lang('common_ebt') => lang('common_ebt'),
                    lang('common_ebt_cash') => lang('common_ebt_cash')
                ));
            }
        } else {
            $payment_options = array(
    ...
    function get_unpaid_store_account_sale_ids($customer_id, $limit = 30)
    {
