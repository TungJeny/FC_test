...
class Giftcard extends CI_Model
{
    ...
    {
        $this->db->where('giftcard_id', $giftcard_id);
        return $this->db->update('giftcards', array(
            'deleted' => 1,
    ...
    {
        $this->db->where('giftcard_number', $giftcard_number);
        $this->db->update('giftcards', array(
            'value' => $value
        ));
    }
    ...
    function log_modification($data)
    {
        ...
        if ($data['type'] == "sale") {
            ...
            if (isset($data['sale_id'])) {
                $sale_id_message = lang('common_sale_id') . ': ' . anchor('sales/receipt/' . $data['sale_id'], $this->config->item('sale_prefix') . ' ' . $data['sale_id'], array(
                    'target' => '_blank'
                )) . ' ';
            }
        ...
        $this->db->insert('giftcards_log', array(
