...
class Receiving extends CI_Model
{
    ...
    function save($items, $supplier_id, $employee_id, $comment, $payments, $receiving_id = false, $suspended = 0, $mode = 'receive', $change_receiving_date = false, $is_po = 0, $location_id = -1, $balance = 0, $store_account_payment = 0)
    {
        ...
        foreach ($items as $line => $item) {
            ...
            $recv_item_tax = $recv_total - $recv_subtotal;
    ...
    function delete($receiving_id, $all_data = false, $update_quantity = true)
    {
        ...
        if ($all_data) {
            ...
            $this->db->delete('receivings_items_taxes', array(
                'receiving_id' => $receiving_id
            ));
            $this->db->delete('receivings_payments', array(
                'receiving_id' => $receiving_id
            ));
        }
        
        $this->db->where('receiving_id', $receiving_id);
        return $this->db->update('receivings', array(
            'deleted' => 1,
    ...
    function get_supplier($receiving_id)
    {
        $this->db->from('receivings');
        $this->db->where('receiving_id', $receiving_id);
        return $this->Supplier->get_info($this->db->get()
            ->row()->supplier_id);
    }
        ...
        if ($current_receivings_items_data['quantity_purchased'] > 0) {
            ...
            if ($averaging_method == 'moving_average') {
                ...
                if ($cur_item_location_info && $cur_item_location_info->cost_price) {
					...
					FROM " . $this->db->dbprefix('receivings_items') . ' ' . 'JOIN ' . $this->db->dbprefix('receivings') . ' ON ' . $this->db->dbprefix('receivings') . '.receiving_id = ' . $this->db->dbprefix('receivings_items') . '.receiving_id ' . 'WHERE quantity_purchased > 0 and item_id=' . $this->db->escape($item_id) . ' and location_id = ' . $this->db->escape($location_id))
                        ->result();
                } else {
                    $result = $this->db->query("SELECT ROUND((SUM(item_unit_price*quantity_purchased-item_unit_price*quantity_purchased*discount_percent/100)) / SUM(quantity_purchased),10) as cost_price_average 
					FROM " . $this->db->dbprefix('receivings_items') . '
					WHERE quantity_purchased > 0 and item_id=' . $this->db->escape($item_id))
                        ->result();
                }
    ...
    function get_receiving_items_taxes($receiving_id, $line = FALSE)
    {
        ...
        $query = $this->db->query('SELECT name, percent, cumulative, item_unit_price as price, quantity_purchased as quantity, discount_percent as discount ' . 'FROM ' . $this->db->dbprefix('receivings_items_taxes') . ' JOIN ' . $this->db->dbprefix('receivings_items') . ' USING (receiving_id, item_id, line) ' . 'WHERE ' . $this->db->dbprefix('receivings_items_taxes') . ".receiving_id = $receiving_id" . ' ' . $item_where . ' ' . 'ORDER BY ' . $this->db->dbprefix('receivings_items') . '.line,' . $this->db->dbprefix('receivings_items') . '.item_id,cumulative,name,percent');
    ...
    function get_unpaid_store_account_recv_ids($supplier_id, $limit = 30)
    {
        ...
        $this->db->where('receiving_id IS NOT NULL');
        ...
        foreach ($all_payments_for_receivings as $receiving_id => $payment_rows) {
            if (isset($receivings_totals[$receiving_id])) {
                ...
                foreach ($payment_rows as $payment_row) {
                    ...
                    if (($total_receiving_balance != 0 || ($receivings_totals[$receiving_id] >= 0 && $payment_row['payment_amount'] < 0) || ($receivings_totals[$receiving_id] < 0 && $payment_row['payment_amount'] >= 0)) && $exists) {
                ...
                foreach ($payment_rows as $payment_row) {
                    ...
                    if (! isset($foreign_language_to_cur_language[$payment_row['payment_type']]) || ! isset($payment_data[$receiving_id][$foreign_language_to_cur_language[$payment_row['payment_type']]])) {
                        ...
                        if (strpos($payment_row['payment_type'], ':') !== FALSE && ! isset($foreign_language_to_cur_language[$payment_row['payment_type']])) {
                            list ($giftcard_translation, $giftcard_number) = explode(":", $payment_row['payment_type']);
                            $foreign_language_to_cur_language[$payment_row['payment_type']] = $foreign_language_to_cur_language[$giftcard_translation] . ':' . $giftcard_number;
                            $payment_data[$receiving_id][$foreign_language_to_cur_language[$payment_row['payment_type']]] = array(
                                'receiving_id' => $receiving_id,
                                'payment_type' => $foreign_language_to_cur_language[$payment_row['payment_type']],
                                'payment_amount' => 0,
                                'payment_date' => $payment_row['payment_date'],
                                'receiving_time' => $payment_row['receiving_time']
                            );
                            ...
                            $payment_data[$receiving_id][$payment_row['payment_type']] = array(
                                'receiving_id' => $receiving_id,
                                'payment_type' => $payment_row['payment_type'],
                                'payment_amount' => 0,
                                'payment_date' => $payment_row['payment_date'],
