...
class Inventory extends CI_Model
{
    ...
    {
        $this->db->delete('inventory_counts_items', array(
            'inventory_counts_id' => $count_id,
            'item_id' => $item_id
        ));
    }
    ...
    function delete_inventory_count($count_id)
    {
        ...
        $this->db->delete('inventory_counts', array(
            'id' => $count_id
        ));
    ...
    public function get_history_trans($ids = [], $options = [])
    {
        ...
        if (! empty($options['start_date'])) {
            $this->db->where('inventory.trans_date >= ', $options['start_date']);
        }
        
        if (! empty($options['end_date'])) {
            $this->db->where('inventory.trans_date <= ', $options['end_date']);
        }
        
        if (! empty($options['location_ids'])) {
            $this->db->where_in('inventory.location_id', $options['location_ids']);
        }
        ...
        if (! empty($options['tag'])) {
            ...
            $this->db->where('tags.id', $options['tag']);
        }
