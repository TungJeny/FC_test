...
class Category extends CI_Model
{
    ...
    function get_all_categories_and_sub_categories_as_tree()
    {
        ...
        return array_filter($objects, array(
            'Category',
            '_filter_to_root'
        ));
    }
    ...
    function sort_categories_and_sub_categories($categories)
    {
        ...
        $sorted = array_filter($objects, array(
            'Category',
            '_filter_to_root'
        ));
        ...
        foreach ($categories as $category) {
            $return[$category->id] = array(
                ...
                'name' => $category->name,
                'hide_from_grid' => $category->hide_from_grid,
    ...
    function get_all($parent_id = NULL, $show_hidden = FALSE, $limit = 10000, $offset = 0, $col = 'name', $order = 'asc', $can_use_cache = TRUE)
    {
        ...
        if (! $can_use_cache || ! $cache) {
            ...
            foreach ($this->db->get()->result_array() as $result) {
                $cache[$result['parent_id'] ? $result['parent_id'] : 0][] = array(
                    'name' => $result['name'],
                    'color' => $result['color'],
                    'image_id' => $result['image_id'],
                    'hide_from_grid' => $result['hide_from_grid'],
        ...
        if (isset($cache[$key])) {
            foreach ($cache[$key] as $row) {
                $return[$row['id']] = array(
                    ...
                    'hide_from_grid' => $row['hide_from_grid'],
        ...
        if ($category_id == FALSE) {
            if ($category_name) {
                if ($this->db->insert('categories', array(
                    'name' => $category_name,
                    'hide_from_grid' => $hide_from_grid ? 1 : 0,
                    'parent_id' => $parent_id,
                    'color' => $category_color ? $category_color : '',
    ...
    function delete($category_id)
    {
        ...
        $this->db->where('id', $category_id);
        return $this->db->update('categories', array(
            'deleted' => 1
        ));
    }
