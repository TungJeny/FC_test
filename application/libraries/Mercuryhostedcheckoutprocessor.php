        ...
        if (! $this->controller->sale_lib->get_use_saved_cc_info()) {
            ...
            try {
                ...
                if ($response_code == 0) {
                    ...
                    $this->controller->load->view('sales/mercury_hosted_checkout', array(
        ...
        try {
            ...
            if ($response_code == 0 && $status == 'Approved') {
                ...
                if ($response_code == 0 && $auth_amount == $total_amount) {
                    ...
                    $this->controller->_reload(array(
                        'error' => lang('sales_acknowledge_payment_failed_please_contact_support')
                    ), false);
                }
            } else {
                ...
                if ($status == 'Declined') {
                    ...
                    $this->controller->_reload(array(
                        'error' => $result->VerifyPaymentResult->StatusMessage . ': ' . $result->VerifyPaymentResult->DisplayMessage
                    ), false);
                }
            }
        } catch (SoapFault $e) {
