...
class MY_Migration extends CI_Migration
{
    ...
    public function need_to_migrate()
    {
        ...
        $migrations = array_keys($this->find_migrations());
        $max_version = end($migrations);
    ...
    {
        if (is_callable('system') && false === stripos(ini_get('disable_functions'), 'system')) {
            // 1st method; directly via mysql
            $mysql_paths = array();
            
            // use mysql location from `which` command.
            
            if (! is_executable('C:\PHPPOS\mysql\bin\mysql.exe') && ! is_executable('C:\Program Files\LifeTek Stack\mysql\bin\mysql.exe')) {
            ...
            // Don't include windows paths because it causes DOS windows to show up in installer
            // $mysql_paths[] = 'C:\Program Files\LifeTek Stack\mysql\bin\mysql.exe'; //Windows
