...
class Sale_lib
{
        ...
        if ($number_of_discounted_items > 0) {
            ...
            if ($discounted_item_quantity_to_be_added !== 0) {
                ...
                if ($kit) {
                    ...
                    $this->add_item($id, $discounted_item_quantity_to_be_added, $discount, $price, null, null, null, null, TRUE, $discounted_item_line, TRUE, FALSE, $rule);
                }
    ...
    function apply_spend_x_get_discount($rule)
    {
        ...
        $this->add_item($item_id, $neg_apply, 0, to_currency_no_money($discount_amount), to_currency_no_money($discount_amount), 0, null, null, false, false, true, false, $rule);
    }
    ...
    function do_spending_price_rule($params)
    {
        ...
        $rule = $this->CI->Price_rule->get_price_rule_for_spending($sub_total);
    ...
    function add_item($item_id, $quantity = 1, $discount = 0, $price = null, $regular_price = null, $cost_price = null, $description = null, $serialnumber = null, $force_add_or_update = FALSE, $line = FALSE, $update_register_cart_data = TRUE, $apply_price_rules = TRUE, $rule = array())
    {
        ...
        foreach ($items as $item) {
            ...
            if (isset($item['item_id']) && $item['item_id'] == $item_id && (empty($item['rule']) || ($force_add_or_update || isset($item['rule']['type']) && (! in_array($item['rule']['type'], array(
                'buy_x_get_discount',
                'buy_x_get_y_free',
                'simple_discount'
            )))))) // We skip items with buy_x_get_y_free as we don't want to update these normally unless $force_add_or_update
            {
    ...
    function add_item_kit($external_item_kit_id_or_item_number, $quantity = 1, $discount = 0, $price = null, $regular_price = null, $cost_price = null, $description = null, $force_add_or_update = FALSE, $line = FALSE, $update_register_cart_data = TRUE, $apply_price_rules = TRUE, $rule = array())
    {
        ...
        if ($item_kit_info->unit_price == null) {
            foreach ($this->CI->Item_kit_items->get_info($item_kit_id) as $item_kit_item) {
                ...
                if ((string) (int) $item_kit_item->quantity == $item_kit_item->quantity) {
                    for ($k = 0; $k < $item_kit_item->quantity; $k ++) {
                        $this->add_item($item_kit_item->item_id, $quantity, 0, null, null, null, null, null, $force_add_or_update, FALSE);
                    }
                } else {
                    $this->add_item($item_kit_item->item_id, $item_kit_item->quantity, 0, null, null, null, null, null, $force_add_or_update, FALSE);
                }
            }
            ...
            foreach ($items as $item) {
                ...
                if (isset($item['item_kit_id']) && $item['item_kit_id'] == $item_kit_id && (empty($item['rule']) || ($force_add_or_update || isset($item['rule']['type']) && (! in_array($item['rule']['type'], array(
                    'buy_x_get_discount',
                    'buy_x_get_y_free',
                    'simple_discount'
                )))))) // We skip items with price rules discount as we don't want to update these normally unless $force_add_or_update is true
                {
                    $itemalreadyinsale = TRUE;
                    $updatekey = $item['line'];
                }
    ...
    function return_entire_sale($receipt_sale_id)
    {
        ...
        foreach ($this->CI->Sale->get_sale_items($sale_id)->result() as $row) {
            ...
            $this->add_item($row->item_id, - $row->quantity_purchased, $row->discount_percent, $price_to_use, $row->regular_item_unit_price_at_time_of_sale, $row->item_cost_price, $row->description, $row->serialnumber, TRUE, $row->line, FALSE, FALSE);
        }
    ...
    function copy_entire_sale($sale_id, $is_receipt = false)
    {
        ...
        foreach ($this->CI->Sale->get_sale_items($sale_id)->result_array() as $row) {
            ...
            $this->add_item($row['item_id'], $row['quantity_purchased'], $row['discount_percent'], $price_to_use, $row['regular_item_unit_price_at_time_of_sale'], $row['item_cost_price'], $row['description'], $row['serialnumber'], TRUE, $row['line'], FALSE, FALSE, $rule);
        }
