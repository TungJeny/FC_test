    ...
    foreach ($item_kit_ids as $item_kit_id) {
        ...
        if ($CI->config->item('barcode_price_include_tax')) {
            if ($item_kit_info->tax_included) {
                $result[] = array(
                    'name' => (! $hide_prices ? '<span style="font-size:10pt;font-weight:bold;">' . to_currency($item_kit_price) . '</span>: ' : '') . $item_kit_info->name,
                    'id' => $barcode_number
                );
            } else {
                $result[] = array(
                    'name' => (! $hide_prices ? '<span style="font-size:10pt;font-weight:bold;">' . to_currency(get_price_for_item_kit_including_taxes($item_kit_id, $item_kit_price)) . '</span>: ' : '') . $item_kit_info->name,
                    'id' => $barcode_number
                );
            }
        } else {
            if ($item_kit_info->tax_included) {
                $result[] = array(
                    'name' => (! $hide_prices ? '<span style="font-size:10pt;font-weight:bold;">' . to_currency(get_price_for_item_kit_excluding_taxes($item_kit_id, $item_kit_price)) . '</span>: ' : '') . $item_kit_info->name,
                    'id' => $barcode_number
                );
            } else {
                $result[] = array(
