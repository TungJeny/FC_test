    ...
    foreach ($item_ids as $item_id) {
        ...
        if ($CI->config->item('barcode_price_include_tax')) {
            if ($item_info->tax_included) {
                $result[] = array(
                    'name' => ! $hide_prices ? (($is_item_location_promo || $is_item_promo ? '<span style="text-decoration: line-through;font-size:10pt;font-weight:bold;">' . to_currency($regular_item_price) . '</span> ' : ' ') . '<span style="font-size:10pt;font-weight:bold;">' . to_currency($item_price) . '</span>: ' . $item_info->name . ($item_info->size ? ' (' . $item_info->size . ')' : '')) : $item_info->name . ($item_info->size ? ' (' . $item_info->size . ')' : ''),
                    'id' => $barcode_number
                );
            } else {
                $result[] = array(
                    'name' => ! $hide_prices ? (($is_item_location_promo || $is_item_promo ? '<span style="text-decoration: line-through;font-size:10pt;font-weight:bold;">' . to_currency(get_price_for_item_including_taxes($item_id, $regular_item_price)) . '</span> ' : ' ') . '<span style="font-size:10pt;font-weight:bold;">' . to_currency(get_price_for_item_including_taxes($item_id, $item_price)) . '</span>: ' . $item_info->name . ($item_info->size ? ' (' . $item_info->size . ')' : '')) : $item_info->name . ($item_info->size ? ' (' . $item_info->size . ')' : ''),
                    'id' => $barcode_number
                );
            }
        } else {
            if ($item_info->tax_included) {
                $result[] = array(
                    'name' => ! $hide_prices ? (($is_item_location_promo || $is_item_promo ? '<span style="text-decoration: line-through;font-size:10pt;font-weight:bold;">' . to_currency(get_price_for_item_excluding_taxes($item_id, $regular_item_price)) . '</span> ' : ' ') . '<span style="font-size:10pt;font-weight:bold;">' . to_currency(get_price_for_item_excluding_taxes($item_id, $item_price)) . '</span>: ' . $item_info->name . ($item_info->size ? ' (' . $item_info->size . ')' : '')) : $item_info->name . ($item_info->size ? ' (' . $item_info->size . ')' : ''),
                    'id' => $barcode_number
                );
            } else {
                $result[] = array(
