...
function create_breadcrumb()
{
    ...
    } elseif ($ci->uri->segment(1) == 'materials') {
        
        if ($ci->uri->segment(2) == NULL)
        {
            $materials_home_link = create_current_page_url(lang('module_mrp_item_bom'));
        } else {
            $materials_home_link = '<a tabindex = "-1" href="' . site_url('materials/boms') . '">' . lang('module_mrp_item_bom_home') . '</a>';
        }
        $return .= $materials_home_link;
        if ($ci->uri->segment(2) == 'view') {
            $return .= create_current_page_url(lang('module_mrp_item_bom_view'));
        }
    }
    return $return;
}
