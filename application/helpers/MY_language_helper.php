...
{
    $CI = & get_instance();
    return ($CI->Employee->is_logged_in() && $CI->Employee->get_logged_in_employee_info()->language == 'arabic') || ($CI->Employee->is_logged_in() && ! $CI->Employee->get_logged_in_employee_info()->language && $CI->config->item('language') == 'arabic') || ! $CI->Employee->is_logged_in() && $CI->config->item('language') == 'arabic';
}
...
function lang($line, $for = '', $attributes = array())
{
    ...
    if (! $line) {
        ...
        if (! $line) {
            ...
            log_message('error', $log_message);
            if (ENVIRONMENT == 'development' && WARNING_LANGUAGE_TEXT) {
