...
class Suppliers extends Person_controller
{
    ...
    {
        $params = $this->session->userdata('supplier_search_data') ? $this->session->userdata('supplier_search_data') : array(
            'offset' => 0,
    ...
    function excel()
    {
        ...
        array_to_spreadsheet(array(
            $header_row
        ), 'import_suppliers.' . ($this->config->item('spreadsheet_format') == 'XLSX' ? 'xlsx' : 'csv'));
    }
    ...
    function do_excel_import()
    {
        ...
        if ($_FILES['file_path']['error'] != UPLOAD_ERR_OK) {
            ...
            if (($handle = fopen($_FILES['file_path']['tmp_name'], "r")) !== FALSE) {
                ...
                for ($k = 2; $k <= $num_rows; $k ++) {
                    ...
                    if ($this->config->item('suppliers_store_accounts')) {
                        $balance = $sheet->getCellByColumnAndRow(13, $k);
                        if (! $balance) {
                            $balance = 0;
                        }
                        $person_id = $sheet->getCellByColumnAndRow(14, $k);
                    } else {
                        $balance = 0;
                        $person_id = $sheet->getCellByColumnAndRow(13, $k);
                    }
                    ...
                    if (! $this->Supplier->save_supplier($person_data, $supplier_data, $person_id ? $person_id : FALSE)) {
                        echo json_encode(array(
                            'success' => false,
                            'message' => lang('suppliers_duplicate_account_id')
                        ));
                        return;
                    }
                }
        ...
        echo json_encode(array(
            'success' => true,
            'message' => lang('suppliers_import_successfull')
        ));
    }
    ...
    function excel_export()
    {
        ...
        if ($search) {
            $data = $this->Supplier->search($search, $this->Supplier->search_count_all($search), 0, $params['order_col'], $params['order_dir'])
                ->result_object();
        } else {
            $data = $this->Supplier->get_all($this->Supplier->count_all())
                ->result_object();
        }
    ...
    function delete()
    {
        ...
        if ($this->Supplier->delete_list($suppliers_to_delete)) {
            echo json_encode(array(
                'success' => true,
                'message' => lang('suppliers_successful_deleted') . ' ' . count($suppliers_to_delete) . ' ' . lang('suppliers_one_or_multiple')
            ));
        } else {
            echo json_encode(array(
                'success' => false,
                'message' => lang('suppliers_cannot_be_deleted')
            ));
        }
    }
    ...
    function cleanup()
    {
        ...
        echo json_encode(array(
            'success' => true,
            'message' => lang('suppliers_cleanup_sucessful')
        ));
    }
