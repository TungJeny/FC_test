...
class Sales extends Secure_area
{
    ...
    function index($dont_switch_employee = 0)
    {
        ...
        if ($this->config->item('track_cash')) {
            if ($this->input->post('opening_amount') != '' && ! $this->Register->is_register_log_open()) {
                ...
                $this->_reload(array(
                    'dont_switch_employee' => $dont_switch_employee
                ), false);
            } else {
                $this->load->view('sales/opening_amount', array(
                    'previous_closing' => $this->Register->get_previous_closing_amount($this->Employee->get_logged_in_employee_current_register_id()),
                    'denominations' => $this->Register->get_register_currency_denominations()
                        ->result_array()
                ));
            }
        } else {
        ...
        if ($redirect) {
            ...
            if ($previous_mode == 'sale' && $mode == 'return') {
                if ($this->sale_lib->can_convert_cart_from_sale_to_return()) {
                    $data = array(
                        'prompt_convert_sale_to_retun' => TRUE
                    );
                } else {
                    $data = array(
                        'prompt_convert_sale_to_retun' => FALSE
                    );
                }
            }
            $this->_reload($data);
    ...
    function add_payment()
    {
        ...
        if (($this->input->post('payment_type') == lang('common_store_account') && $this->sale_lib->get_customer() == - 1) || ($this->sale_lib->get_mode() == 'store_account_payment' && $this->sale_lib->get_customer() == - 1)) {
        ...
        if ($this->input->post('payment_type') == lang('common_ebt') && $this->input->post('amount_tendered') > $this->sale_lib->get_ebt_total_amount_to_charge()) {
    ...
    function add()
    {
        ...
        if ($this->sale_lib->is_valid_receipt($item_id_or_number_or_item_kit_or_receipt) && $mode == 'return') {
            ...
            if ($this->Item_kit->get_info(str_replace(array(
                'kit ',
                'KIT '
            ), '', $item_id_or_number_or_item_kit_or_receipt))->deleted || $this->Item_kit->get_info($this->Item_kit->get_item_kit_id(str_replace(array(
                'kit ',
                'KIT '
            ), '', $item_id_or_number_or_item_kit_or_receipt)))->deleted) {
        ...
        } elseif ($this->Item->get_info($item_id_or_number_or_item_kit_or_receipt)->deleted || $this->Item->get_info($this->Item->get_item_id($item_id_or_number_or_item_kit_or_receipt))->deleted || ! $this->sale_lib->add_item($item_id_or_number_or_item_kit_or_receipt, $quantity, $discount = 0, null, null, null, null, $serialnumber)) {
            $out_of_stock_check = FALSE;
    ...
    function start_cc_processing()
    {
        if ($this->config->item('test_mode')) {
            $this->_reload(array(
                'error' => lang('common_in_test_mode')
            ), false);
            return;
        }
        ...
        if ($total >= 0 && $cc_amount > $total) {
            $this->_reload(array(
                'error' => lang('sales_credit_card_payment_is_greater_than_total_cannot_complete')
            ), false);
        } elseif ($total < 0 && $cc_amount < $total) {
            $this->_reload(array(
                'error' => lang('sales_cannot_refund_more_than_sale_total')
            ), false);
        } else {
            ...
            if ($credit_card_processor) {
                ...
                $this->_reload(array(
                    'error' => lang('sales_credit_card_processing_is_down')
                ), false);
                return;
            }
        }
    ...
    function complete()
    {
        ...
        if (! $this->_payments_cover_total()) {
            $this->_reload(array(
                'error' => lang('sales_cannot_complete_sale_as_payments_do_not_cover_total')
            ), false);
            return;
        }
        ...
        foreach ($store_account_in_all_languages as $store_account_lang) {
            $data['balance'] += $this->sale_lib->get_payment_amount($store_account_lang);
        }
        ...
        $data['rule'] = $this->_get_spending_discount($data['cart']);
        ...
        $old_date = $this->sale_lib->get_change_sale_id() ? $this->Sale->get_info($this->sale_lib->get_change_sale_id())
            ->row_array() : false;
        ...
        $sale_id_raw = $this->Sale->save($data['cart'], $customer_id, $employee_id, $sold_by_employee_id, $data['comment'], $data['discount_reason'], $data['show_comment_on_receipt'], $data['payments'], $suspended_change_sale_id, 0, $data['change_sale_date'], $data['balance'], $data['store_account_payment'], $data['rule']);
        ...
        $this->Register_cart->add_data(array(
            'can_email' => ! $this->sale_lib->get_email_receipt(),
            'sale_id' => $sale_id_raw
        ), $this->Employee->get_logged_in_employee_current_register_id());
    ...
    function receipt_validate()
    {
        ...
        if (! $sale_info) {
            echo json_encode(array(
                'success' => false,
                'message' => lang('sales_sale_id_not_found')
            ));
            die();
        } else {
            echo json_encode(array(
    ...
    /* Muazzam */
    function _get_spending_discount($cart)
    {
        foreach ($cart as $line => $item) {
            if (isset($item['rule']['spending_discount']) && isset($item['rule']['rule_id']) && $item['rule']['spending_discount'] > 0) {
                return $item['rule'];
            }
        }
    ...
    function delete_sale_only($sale_id)
    {
        ...
        if ($this->Sale->delete($sale_id)) {
            echo json_encode(array(
                'success' => true,
                'message' => lang('sales_successfully_deleted')
            ));
        } else {
            echo json_encode(array(
                'success' => true,
                'message' => lang('sales_unsuccessfully_deleted')
            ));
        }
    }
    ...
    function delete($sale_id)
    {
        ...
        if (! $this->input->post('do_delete')) {
            $this->load->view('sales/delete', array(
                'success' => false
            ));
            return;
        }
    ...
    function undelete($sale_id)
    {
        if (! $this->input->post('do_undelete')) {
            $this->load->view('sales/undelete', array(
                'success' => false
            ));
            return;
        }
    ...
    function save($sale_id)
    {
        ...
        if ($this->Sale->update($sale_data, $sale_id)) {
            echo json_encode(array(
                'success' => true,
                'message' => lang('sales_successfully_updated')
            ));
        } else {
            echo json_encode(array(
                'success' => false,
                'message' => lang('sales_unsuccessfully_updated')
            ));
        }
    }
    ...
    function _reload($data = array(), $is_ajax = true)
    {
        ...
        $modes = array(
            'sale' => lang('sales_sale'),
            'return' => lang('sales_return')
        );
        ...
        if ($customer_id != - 1) {
            ...
            $data['sales_until_discount'] = (float) $this->config->item('number_of_sales_for_discount') - (float) $cust_info->current_sales_for_discount;
        }
        ...
        if ($this->Location->get_info_for_key('enable_credit_card_processing')) {
            ...
            if ($credit_card_processor && method_exists($credit_card_processor, 'void_partial_transactions')) {
                if (! $credit_card_processor->void_partial_transactions()) {
                    $this->sale_lib->clear_all();
                    $this->_reload(array(
                        'error' => lang('sales_attempted_to_reverse_transactions_failed_please_contact_support')
                    ), true);
                    return;
                }
            }
    ...
    function suspend($suspend_type = 1)
    {
        ...
        // SAVE sale to database
        $sale_id = $this->Sale->save($data['cart'], $customer_id, $employee_id, $sold_by_employee_id, $comment, $discount_reason, $show_comment_on_receipt, $data['payments'], $sale_id, $suspend_type, $sale_date, $data['balance']);
        
        $data['sale_id'] = $this->config->item('sale_prefix') . ' ' . $sale_id;
        if ($data['sale_id'] == $this->config->item('sale_prefix') . ' -1') {
            $this->_reload(array(
                'error' => lang('sales_transaction_failed')
            ));
            return;
        }
        $this->sale_lib->clear_all();
        ...
        if ($this->config->item('show_receipt_after_suspending_sale')) {
            ...
            $this->_reload(array(
                'success' => lang('sales_successfully_suspended_sale')
            ));
        }
    }
    ...
    function excel()
    {
        ...
        array_to_spreadsheet(array(
            $header_row
        ), 'batch_sale_export.' . ($this->config->item('spreadsheet_format') == 'XLSX' ? 'xlsx' : 'csv'));
    }
    ...
    function do_excel_import()
    {
                ...
                for ($k = 2; $k <= $num_rows; $k ++) {
                    ...
                    if ($this->sale_lib->is_valid_item_kit($item_id)) {
                        if (! $this->sale_lib->add_item_kit($item_id, $quantity, $discount, $price)) {
                            ...
                            echo json_encode(array(
                                'success' => false,
                                'message' => lang('batch_sales_error')
                            ));
                            return;
                        }
        ...
        echo json_encode(array(
            'success' => true,
            'message' => lang('sales_import_successfull')
        ));
    }
    ...
    function change_sale($sale_id)
    {
        ...
        $this->_reload(array(
            'orig_location' => $sale_info->location_id
        ), false);
    }
    ...
    function discount_all()
    {
        ...
        if ($this->input->post('name') == "discount_all_percent") {
            ...
            $this->sale_lib->add_item($item_id, - 1, 0, to_currency_no_money($discount_amount), to_currency_no_money($discount_amount), 0, $description, null, false, false, true, false);
        }
    ...
    function categories_and_items($category_id = NULL, $offset = 0)
    {
        ...
        $items_result = $this->Item->get_all_by_category($category_id, $this->config->item('hide_out_of_stock_grid') ? TRUE : FALSE, $items_offset, $this->config->item('number_of_items_in_grid') ? $this->config->item('number_of_items_in_grid') : 14)
            ->result();
    ...
    function tag_items($tag_id, $offset = 0)
    {
        ...
        $items_result = $this->Item->get_all_by_tag($tag_id, $this->config->item('hide_out_of_stock_grid') ? TRUE : FALSE, $offset, $this->config->item('number_of_items_in_grid') ? $this->config->item('number_of_items_in_grid') : 14)
            ->result();
    ...
    function sig_save()
    {
        ...
        $this->Sale->update(array(
            'signature_image_id' => $image_file_id
        ), $sale_id);
        
        echo json_encode(array(
            'file_id' => $image_file_id,
            'file_timestamp' => $this->Appfile->get_file_timestamp($image_file_id)
        ));
    }
    ...
    function customer_display($register_id = false)
    {
        ...
        if ($this->Register->exists($register_id)) {
            ...
            $this->load->view('sales/customer_display_initial', array(
