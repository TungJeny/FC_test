...
class Customers extends Person_controller
{
    ...
    {
        $params = $this->session->userdata('customers_search_data') ? $this->session->userdata('customers_search_data') : array(
            'offset' => 0,
            'order_col' => 'last_name',
            'order_dir' => 'asc',
            'search' => FALSE
    ...
    function save($customer_id = -1)
    {
        ...
        } else // failure
        {
            echo json_encode(array(
                'success' => false,
                'message' => lang('customers_error_adding_updating') . ' ' . $person_data['first_name'] . ' ' . $person_data['last_name'],
                'person_id' => - 1
            ));
        }
    }
    ...
    function delete()
    {
        ...
        if ($this->Customer->delete_list($customers_to_delete)) {
            echo json_encode(array(
                'success' => true,
                'message' => lang('customers_successful_deleted') . ' ' . count($customers_to_delete) . ' ' . lang('customers_one_or_multiple')
            ));
        } else {
            echo json_encode(array(
                'success' => false,
                'message' => lang('customers_cannot_be_deleted')
            ));
        }
    }
    ...
    function _excel_get_header_row()
    {
        $return = array(
            ...
            lang('customers_tax_certificate'),
    ...
    function excel()
    {
        ...
        array_to_spreadsheet(array(
            $header_row
        ), 'import_customers.' . ($this->config->item('spreadsheet_format') == 'XLSX' ? 'xlsx' : 'csv'));
    }
    ...
    {
        echo json_encode(array(
            'duplicate' => $this->Customer->check_duplicate($this->input->post('name'), $this->input->post('email'), $this->input->post('phone_number'))
        ));
    }
    ...
    function excel_export()
    {
        ...
        if ($search) {
            $data = $this->Customer->search($search, $this->Customer->search_count_all($search), 0, $params['order_col'], $params['order_dir'])
                ->result_object();
        } else {
            $data = $this->Customer->get_all($this->Customer->count_all())
                ->result_object();
        }
    ...
    private function _get_database_fields_for_import_as_array()
    {
        ...
        $fields[] = array(
            'Name' => lang('common_first_name'),
            'key' => 'first_name'
        );
        $fields[] = array(
            'Name' => lang('common_last_name'),
            'key' => 'last_name'
        );
        $fields[] = array(
            'Name' => lang('common_email'),
            'key' => 'email'
        );
        $fields[] = array(
            'Name' => lang('common_phone_number'),
            'key' => 'phone_number'
        );
        $fields[] = array(
            'Name' => lang('common_address_1'),
            'key' => 'address_1'
        );
        $fields[] = array(
            'Name' => lang('common_address_2'),
            'key' => 'address_2'
        );
        $fields[] = array(
            'Name' => lang('common_city'),
            'key' => 'city'
        );
        $fields[] = array(
            'Name' => lang('common_state'),
            'key' => 'state'
        );
        $fields[] = array(
            'Name' => lang('common_zip'),
            'key' => 'zip'
        );
        $fields[] = array(
            'Name' => lang('common_country'),
            'key' => 'country'
        );
        $fields[] = array(
            'Name' => lang('common_comments'),
            'key' => 'comments'
        );
        $fields[] = array(
            'Name' => lang('customers_account_number'),
            'key' => 'account_number'
        );
        $fields[] = array(
            'Name' => lang('customers_taxable'),
            'key' => 'taxable'
        );
        $fields[] = array(
            'Name' => lang('customers_tax_certificate'),
            'key' => 'tax_certificate'
        );
        $fields[] = array(
            'Name' => lang('customers_company_name'),
            'key' => 'company_name'
        );
        $fields[] = array(
            'Name' => lang('common_tier_name'),
            'key' => 'tier_id'
        );
        
        if ($this->config->item('customers_store_accounts')) {
            $fields[] = array(
                'Name' => lang('common_balance'),
                'key' => 'balance'
            );
            $fields[] = array(
                'Name' => lang('common_credit_limit'),
                'key' => 'credit_limit'
            );
        }
        ...
        if ($this->config->item('enable_customer_loyalty_system') && $this->config->item('loyalty_option') == 'advanced') {
            $fields[] = array(
                'Name' => lang('common_points'),
                'key' => 'points'
            );
            $fields[] = array(
                'Name' => lang('customers_amount_to_spend_for_next_point'),
                'key' => 'current_spend_for_points'
            );
            $fields[] = array(
        ...
        $fields[] = array(
            'Name' => lang('customers_customer_id'),
            'key' => 'person_id'
        );
    ...
    function dedup_excel_import_data()
    {
        ...
        if (count($account_num_dups) > 0) {
            echo json_encode(array(
                'type' => 'error',
                'message' => lang('customers_duplicate_account_numbers'),
                'title' => lang('common_error')
            ));
        } else {
            echo json_encode(array(
                'type' => 'success',
                'message' => lang('customers_no_duplicate_account_numbers'),
    ...
    function complete_excel_import()
    {
        ...
        for ($i = 0; $i < $numRows - 1; $i ++) {
            ...
            if ($person_data['first_name'] && ! $this->Customer->save_customer($person_data, $customer_data, $person_id ? $person_id : FALSE)) {
                ...
                } else { // first attempt failed even with customer id
                    $this->_logDbError($i + 2);
                    $can_commit = FALSE;
                    continue;
                }
    ...
    function cleanup()
    {
        ...
        echo json_encode(array(
            'success' => true,
            'message' => lang('customers_cleanup_sucessful')
        ));
    }
