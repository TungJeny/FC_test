...
class Reports extends Secure_area
{
    ...
    function __construct()
    {
        ...
        $this->load->vars(array(
            'reports_selected_location_ids' => Report::get_selected_location_ids()
        ));
    ...
    function sales_generator()
    {
                ...
                if ($this->input->get('term') != '') { // What exactly are we searchin
                    ...
                    switch ($this->input->get('w')) {
                        case 'customers':
                            ...
                            $t = $this->Customer->search($this->input->get('term'), 100, 0, 'last_name', 'asc')
                                ->result_object();
                            $tmp = array();
                            foreach ($t as $k => $v) {
                        ...
                        case 'employees':
                        case 'salesPerson':
                            $t = $this->Employee->search($this->input->get('term'), 100, 0, 'last_name', 'asc')
                                ->result_object();
                            $tmp = array();
                            foreach ($t as $k => $v) {
                                $tmp[$k] = array(
                                    'id' => $v->person_id,
                                    'name' => $v->last_name . ", " . $v->first_name . " - " . $v->email
                                );
                            }
                            die(json_encode($tmp));
                            break;
                        ...
                        case 'suppliers':
                            ...
                            $t = $this->Supplier->search($this->input->get('term'), 100, 0, 'last_name', 'asc')
                                ->result_object();
                            $tmp = array();
                            foreach ($t as $k => $v) {
                                $tmp[$k] = array(
                                    'id' => $v->person_id,
                                    'name' => $v->last_name . ", " . $v->first_name . " - " . $v->company_name . " - " . $v->email
                                );
                            }
                            die(json_encode($tmp));
                            break;
                        case 'itemsKitName':
                            ...
                            $t = $this->Item_kit->search($this->input->get('term'), FALSE, 100, 0, 'name', 'asc')
                                ->result_object();
                            $tmp = array();
                            foreach ($t as $k => $v) {
                                $tmp[$k] = array(
                                    'id' => $v->item_kit_id,
                                    'name' => $v->name . " / #" . $v->item_kit_number
                                );
                            }
                            die(json_encode($tmp));
                            break;
        ...
        if ($this->input->get('generate_report')) { // Generate Custom Raport
            ...
            foreach (isset($export_excel) == 1 && isset($report_data['summary']) ? $report_data['summary'] : $report_data as $key => $row) {
                ...
                $summary_data_row[] = array(
                    'data' => anchor('sales/receipt/' . $row['sale_id'], '<i class="ion-printer"></i>', array(
                        'target' => '_blank'
                    )) . ' ' . anchor('sales/edit/' . $row['sale_id'], '<i class="ion-document-text"></i>', array(
                        'target' => '_blank'
                    )) . ' ' . anchor('sales/edit/' . $row['sale_id'], lang('common_edit') . ' ' . $row['sale_id'], array(
                        'target' => '_blank'
                    )),
                    'align' => 'left',
                    'sale_id' => $row['sale_id']
                ...
                $summary_data_row[] = array(
                    'data' => date(get_date_format() . '-' . get_time_format(), strtotime($row['sale_time'])),
                    'align' => 'left'
                );
                $summary_data_row[] = array(
                    'data' => $row['register_name'],
                    'align' => 'left'
                );
                $summary_data_row[] = array(
                    'data' => to_quantity($row['items_purchased']),
                    'align' => 'center'
                );
                $summary_data_row[] = array(
                    'data' => $row['employee_name'] . ($row['sold_by_employee'] && $row['sold_by_employee'] != $row['employee_name'] ? '/' . $row['sold_by_employee'] : ''),
                    'align' => 'left'
                );
                $summary_data_row[] = array(
                    'data' => $row['customer_name'] . (isset($row['account_number']) && $row['account_number'] ? ' (' . $row['account_number'] . ')' : ''),
                    'align' => 'left'
                );
                $summary_data_row[] = array(
                ...
                if ($export_excel == 1) {
                    foreach ($report_data['details'][$key] as $drow) {
                        ...
                        $details_data_row[] = array(
                            'data' => $drow['item_number'],
                            'align' => 'left'
                        );
                        $details_data_row[] = array(
                            'data' => $drow['item_product_id'],
                            'align' => 'left'
                        );
                        $details_data_row[] = array(
                            'data' => $drow['item_name'],
                        ...
                        $details_data_row[] = array(
                            'data' => $this->Category->get_full_path($drow['category_id']),
                            'align' => 'left'
                        );
                        $details_data_row[] = array(
                            'data' => $drow['size'],
                        ...
                        $details_data_row[] = array(
                            'data' => $drow['serialnumber'],
                            'align' => 'left'
                        );
                        $details_data_row[] = array(
                            'data' => $drow['description'],
                            'align' => 'left'
                        );
                        $details_data_row[] = array(
                            'data' => to_currency($drow['current_selling_price']),
                            'align' => 'left'
                        );
                        $details_data_row[] = array(
                            'data' => to_quantity($drow['quantity_purchased']),
                            'align' => 'left'
                        );
                        $details_data_row[] = array(
                            'data' => to_currency($drow['subtotal']),
                            'align' => 'right'
                        );
                        $details_data_row[] = array(
                            'data' => to_currency($drow['total']),
                            'align' => 'right'
                        );
                        $details_data_row[] = array(
                            'data' => to_currency($drow['tax']),
                            'align' => 'right'
                        );
                        
                        if ($this->has_profit_permission) {
                            $details_data_row[] = array(
                                'data' => to_currency($drow['profit']),
                                'align' => 'right'
                            );
                        }
                        ...
                        if ($this->has_cost_price_permission) {
                            $details_data_row[] = array(
                                'data' => to_currency($drow['cost_prices']),
                                'align' => 'right'
                            );
                        }
                        
                        $details_data_row[] = array(
                            'data' => $drow['discount_percent'] . '%',
                            'align' => 'left'
                        );
        ...
        {
            $this->load->view("reports/tabular_details_new", $reportdata);
        }
    }
    ...
    function detailed_register_log($start_date, $end_date, $export_excel = 0, $offset = 0)
    {
        ...
        foreach ($report_data as $row) {
            ...
            $summary_data[] = array(
                ...
                array(
                    'data' => $row['open_first_name'] . ' ' . $row['open_last_name'],
                    'align' => 'left'
                ),
                array(
                    'data' => $row['close_first_name'] . ' ' . $row['close_last_name'],
                    'align' => 'left'
                ),
                array(
                    'data' => date(get_date_format(), strtotime($row['shift_start'])) . ' ' . date(get_time_format(), strtotime($row['shift_start'])),
                    'align' => 'left'
                ),
                array(
                    'data' => $shift_end,
                ...
                array(
                    'data' => to_currency($row['open_amount']),
                    'align' => 'right'
                ),
                array(
                    'data' => to_currency($row['close_amount']),
                    'align' => 'right'
                ),
                array(
                    'data' => to_currency($row['cash_sales_amount']),
                    'align' => 'right'
                ),
                array(
                    'data' => to_currency($row['total_cash_additions']),
                    'align' => 'right'
                ),
                array(
                    'data' => to_currency($row['total_cash_subtractions']),
                    'align' => 'right'
                ),
                array(
                    'data' => to_currency($row['difference']),
                    'align' => 'right'
                ),
                array(
                    'data' => $row['notes'],
    ...
    function summary_count_report($start_date, $end_date, $export_excel = 0, $offset = 0)
    {
        ...
        foreach ($report_data as $row) {
            ...
            $tabular_data_row = array(
                array(
                    'data' => date(get_date_format() . ' ' . get_time_format(), strtotime($row['count_date'])),
                    'align' => 'left'
                ),
                array(
                    'data' => $status,
                ...
                array(
                    'data' => to_quantity($row['items_counted']),
                    'align' => 'left'
                ),
                array(
                    'data' => to_quantity($row['difference']),
                    'align' => 'left'
                ),
                array(
                    'data' => to_currency($row['cost_price_difference']),
                    'align' => 'left'
                ),
                array(
                    'data' => $row['comment'],
            ...
            if ($location_count > 1) {
                array_unshift($tabular_data_row, array(
                    'data' => $row['location_name'],
                    'align' => 'left'
                ));
            }
    ...
    function detailed_count_report($start_date, $end_date, $export_excel = 0, $offset = 0)
    {
        ...
        foreach ($report_data['summary'] as $key => $row) {
            ...
            if ($location_count > 1) {
                array_unshift($summary_data_row, array(
                    'data' => $row['location_name'],
                    'align' => 'left'
                ));
            }
            ...
            foreach ($report_data['details'][$key] as $drow) {
                ...
                $details_data_row[] = array(
                    'data' => to_quantity($drow['count']),
                    'align' => 'left'
                );
                $details_data_row[] = array(
                    'data' => to_quantity($drow['actual_quantity']),
                    'align' => 'left'
                );
                $details_data_row[] = array(
                    'data' => to_currency($drow['cost_price']),
                    'align' => 'left'
                );
                $details_data_row[] = array(
                    'data' => to_currency($drow['cost_price_difference']),
                    'align' => 'left'
                );
                $details_data_row[] = array(
                    'data' => $drow['comment'],
    ...
    function summary_sales_time($start_date, $end_date, $do_compare, $compare_start_date, $compare_end_date, $sale_type, $interval, $export_excel = 0)
    {
        ...
        foreach ($report_data as $row) {
            ...
            $data_row[] = array(
                'data' => $row['number_of_transactions'] . ($do_compare && $row_compare ? ' / <span class="compare ' . ($row_compare['number_of_transactions'] >= $row['number_of_transactions'] ? ($row['number_of_transactions'] == $row_compare['number_of_transactions'] ? '' : 'compare_better') : 'compare_worse') . '">' . $row_compare['number_of_transactions'] . '</span>' : ''),
                'align' => 'right'
            );
            $data_row[] = array(
                'data' => to_currency($row['subtotal']) . ($do_compare && $row_compare ? ' / <span class="compare ' . ($row_compare['subtotal'] >= $row['subtotal'] ? ($row['subtotal'] == $row_compare['subtotal'] ? '' : 'compare_better') : 'compare_worse') . '">' . to_currency($row_compare['subtotal']) . '</span>' : ''),
                'align' => 'right'
            );
            $data_row[] = array(
                'data' => to_currency($row['total']) . ($do_compare && $row_compare ? ' / <span class="compare ' . ($row_compare['total'] >= $row['total'] ? ($row['total'] == $row_compare['total'] ? '' : 'compare_better') : 'compare_worse') . '">' . to_currency($row_compare['total']) . '</span>' : ''),
                'align' => 'right'
            );
            $data_row[] = array(
                'data' => to_currency($row['tax']) . ($do_compare && $row_compare ? ' / <span class="compare ' . ($row_compare['tax'] >= $row['tax'] ? ($row['tax'] == $row_compare['tax'] ? '' : 'compare_better') : 'compare_worse') . '">' . to_currency($row_compare['tax']) . '</span>' : ''),
                'align' => 'right'
            );
            
            if ($this->has_profit_permission) {
                $data_row[] = array(
                    'data' => to_currency($row['profit']) . ($do_compare && $row_compare ? ' / <span class="compare ' . ($row_compare['profit'] >= $row['profit'] ? ($row['profit'] == $row_compare['profit'] ? '' : 'compare_better') : 'compare_worse') . '">' . to_currency($row_compare['profit']) . '</span>' : ''),
                    'align' => 'right'
                );
            }
    ...
    function summary_sales($start_date, $end_date, $do_compare, $compare_start_date, $compare_end_date, $sale_type, $export_excel = 0, $offset = 0)
    {
        ...
        foreach ($report_data as $row) {
            ...
            $data_row[] = array(
                'data' => date(get_date_format(), strtotime($row['sale_date'])) . ($do_compare && $row_compare ? ' / <span class="compare ">' . date(get_date_format(), strtotime($row_compare['sale_date'])) . '</span>' : ''),
                'align' => 'left'
            );
            $data_row[] = array(
                'data' => to_currency($row['subtotal']) . ($do_compare && $row_compare ? ' / <span class="compare ' . ($row_compare['subtotal'] >= $row['subtotal'] ? ($row['subtotal'] == $row_compare['subtotal'] ? '' : 'compare_better') : 'compare_worse') . '">' . to_currency($row_compare['subtotal']) . '</span>' : ''),
    ...
    function summary_categories($start_date, $end_date, $do_compare, $compare_start_date, $compare_end_date, $sale_type, $export_excel = 0, $offset = 0)
    {
        ...
        if ($do_compare) {
            ...
            $model_compare->setParams(array(
                ...
                'offset' => $offset,
                'export_excel' => $export_excel,
                'compare_to_categories' => $compare_to_categories
            ));
        ...
        foreach ($report_data as $row) {
            ...
            $data_row[] = array(
                'data' => $this->Category->get_full_path($row['category_id']),
                'align' => 'left'
            );
            $data_row[] = array(
                'data' => to_currency($row['subtotal']) . ($do_compare && $row_compare ? ' / <span class="compare ' . ($row_compare['subtotal'] >= $row['subtotal'] ? ($row['subtotal'] == $row_compare['subtotal'] ? '' : 'compare_better') : 'compare_worse') . '">' . to_currency($row_compare['subtotal']) . '</span>' : ''),
            ...
            $data_row[] = array(
                'data' => floatval($row['item_sold']) . ($do_compare && $row_compare ? ' / <span class="compare ' . ($row_compare['item_sold'] >= $row['item_sold'] ? ($row['item_sold'] == $row_compare['item_sold'] ? '' : 'compare_better') : 'compare_worse') . '">' . floatval($row_compare['item_sold']) . '</span>' : ''),
                'align' => 'right'
            );
            $tabular_data[] = $data_row;
        }
    ...
    function summary_customers($start_date, $end_date, $sale_type, $total_spent_condition = 'any', $total_spent_amount = 0, $export_excel = 0, $offset = 0)
    {
        ...
        $model->setParams(array(
            ...
            'offset' => $offset,
            'export_excel' => $export_excel,
            'total_spent_condition' => $total_spent_condition,
        ...
        foreach ($report_data as $row) {
            ...
            if ($this->config->item('enable_customer_loyalty_system') && $this->config->item('loyalty_option') == 'advanced') {
                $data_row[] = array(
                    'data' => to_currency_no_money($row['points_used']),
                    'align' => 'right'
                );
                $data_row[] = array(
                    'data' => to_currency_no_money($row['points_gained']),
                    'align' => 'right'
                );
                ...
                $data_row[] = array(
                    'data' => to_quantity($sales_until_discount),
                    'align' => 'right'
                );
            }
            $tabular_data[] = $data_row;
    ...
    function summary_items($start_date, $end_date, $do_compare, $compare_start_date, $compare_end_date, $supplier_id = -1, $category_id = -1, $sale_type = 'all', $show_all_items = 0, $export_excel = 0, $offset = 0)
    {
        ...
        if ($do_compare) {
            ...
            $model_compare->setParams(array(
                ...
                'offset' => $offset,
                'export_excel' => $export_excel,
                'compare_to_items' => $compare_to_items,
                'show_all_items' => $show_all_items
            ));
        ...
        foreach ($report_data as $row) {
            ...
            $data_row[] = array(
                'data' => to_currency($row['current_cost_price']),
                'align' => 'right'
            );
            $data_row[] = array(
                'data' => to_currency($row['current_selling_price']),
                'align' => 'right'
            );
            $data_row[] = array(
                'data' => to_quantity($row['quantity']),
                'align' => 'left'
            );
            $data_row[] = array(
                'data' => to_quantity($row['quantity_purchased']) . ($do_compare && $row_compare ? ' / <span class="compare ' . ($row_compare['quantity_purchased'] >= $row['quantity_purchased'] ? ($row['quantity_purchased'] == $row_compare['quantity_purchased'] ? '' : 'compare_better') : 'compare_worse') . '">' . to_quantity($row_compare['quantity_purchased']) . '</span>' : ''),
                'align' => 'left'
            );
            $data_row[] = array(
                'data' => to_currency($row['subtotal']) . ($do_compare && $row_compare ? ' / <span class="compare ' . ($row_compare['subtotal'] >= $row['subtotal'] ? ($row['subtotal'] == $row_compare['subtotal'] ? '' : 'compare_better') : 'compare_worse') . '">' . to_currency($row_compare['subtotal']) . '</span>' : ''),
    ...
    function summary_item_kits($start_date, $end_date, $do_compare, $compare_start_date, $compare_end_date, $sale_type = 'all', $export_excel = 0, $offset = 0)
    {
        ...
        if ($do_compare) {
            ...
            $model_compare->setParams(array(
                ...
                'export_excel' => $export_excel,
                'offset' => $offset,
                'compare_to_item_kits' => $compare_to_item_kits
            ));
    ...
    function summary_taxes_receivings($start_date, $end_date, $do_compare, $compare_start_date, $compare_end_date, $sale_type, $export_excel = 0, $offset = 0)
    {
        ...
        foreach ($report_data as $row) {
            ...
            $tabular_data[] = array(
                ...
                array(
                    'data' => to_currency($row['subtotal']) . ($do_compare && $row_compare ? ' / <span class="compare ' . ($row_compare['subtotal'] >= $row['subtotal'] ? ($row['subtotal'] == $row_compare['subtotal'] ? '' : 'compare_better') : 'compare_worse') . '">' . to_currency($row_compare['subtotal']) : ''),
                    'align' => 'left'
                ),
                array(
                    'data' => to_currency($row['tax']) . ($do_compare && $row_compare ? ' / <span class="compare ' . ($row_compare['tax'] >= $row['tax'] ? ($row['tax'] == $row_compare['tax'] ? '' : 'compare_better') : 'compare_worse') . '">' . to_currency($row_compare['tax']) . '</span>' : ''),
    ...
    function store_account_activity($start_date, $end_date, $export_excel = 0, $offset = 0)
    {
        ...
        foreach ($report_data as $row) {
            $tabular_data[] = array(
                ...
                array(
                    'data' => date(get_date_format() . '-' . get_time_format(), strtotime($row['date'])),
                    'align' => 'left'
                ),
                array(
                    'data' => $row['sale_id'] ? anchor('sales/receipt/' . $row['sale_id'], $this->config->item('sale_prefix') . ' ' . $row['sale_id'], array(
                        'target' => '_blank'
                    )) : '-',
                    'align' => 'center'
                ),
                array(
                    'data' => $row['transaction_amount'] > 0 ? to_currency($row['transaction_amount']) : to_currency(0),
                    'align' => 'right'
                ),
                array(
                    'data' => $row['transaction_amount'] < 0 ? to_currency($row['transaction_amount'] * - 1) : to_currency(0),
                    'align' => 'right'
                ),
                array(
                    'data' => to_currency($row['balance']),
                    'align' => 'right'
                ),
                array(
                    'data' => $row['items'],
    ...
    function summary_payments($start_date, $end_date, $do_compare, $compare_start_date, $compare_end_date, $sale_type, $export_excel = 0, $offset = 0)
    {
        ...
        foreach ($report_data as $row) {
            ...
            $tabular_data[] = array(
                ...
                array(
                    'data' => to_currency($row['payment_amount']) . ($do_compare && $row_compare ? ' / <span class="compare ' . ($row_compare['payment_amount'] >= $row['payment_amount'] ? ($row['payment_amount'] == $row_compare['payment_amount'] ? '' : 'compare_better') : 'compare_worse') . '">' . to_currency($row_compare['payment_amount']) . '</span>' : ''),
                    'align' => 'right'
                )
            );
        }
    ...
    function specific_customer($start_date, $end_date, $customer_id, $sale_type, $export_excel = 0, $offset = 0)
    {
        ...
        foreach (isset($export_excel) == 1 && isset($report_data['summary']) ? $report_data['summary'] : $report_data as $key => $row) {
            ...
            $summary_data_row[] = array(
                'data' => anchor('sales/receipt/' . $row['sale_id'], '<i class="ion-printer"></i>', array(
                    'target' => '_blank',
                    'class' => 'hidden-print'
                )) . '<span class="visible-print">' . $row['sale_id'] . '</span>' . anchor('sales/edit/' . $row['sale_id'], '<i class="ion-document-text"></i>', array(
                    'target' => '_blank'
                )) . ' ' . anchor('sales/edit/' . $row['sale_id'], lang('common_edit') . ' ' . $row['sale_id'], array(
                    'target' => '_blank',
                    'class' => 'hidden-print'
        ...
        $this->load->view("reports/tabular_details_new", $data);
    }
    ...
    function store_account_statements($customer_id = -1, $start_date, $end_date, $hide_items = 0, $pull_payments_by = 'payment_date', $offset = 0)
    {
        ...
        $model->setParams(array(
            ...
            'start_date' => $start_date,
            'end_date' => $end_date,
            'pull_payments_by' => $pull_payments_by
        ));
        $config = array();
    ...
    function get_report_details_sales_generator()
    {
        ...
        $data = $model->get_report_details($ids, $x, $x);
        print_r(json_encode($data));
        exit();
    }
    ...
    function detailed_sales($start_date, $end_date, $sale_type, $export_excel = 0, $offset = 0)
    {
        ...
        foreach (isset($export_excel) == 1 && isset($report_data['summary']) ? $report_data['summary'] : $report_data as $key => $row) {
            ...
            $summary_data_row[] = array(
                'data' => '<a href="' . $link . '" target="_blank">' . $row['customer_name'] . (isset($row['account_number']) && $row['account_number'] ? ' (' . $row['account_number'] . ')' : '') . '</a>',
                'align' => 'left'
            );
            $summary_data_row[] = array(
                'data' => to_currency($row['subtotal']),
            ...
            if ($export_excel == 1) {
                foreach ($report_data['details'][$key] as $drow) {
                    ...
                    $details_data_row[] = array(
                        'data' => $drow['supplier_name'] . ' (' . $drow['supplier_id'] . ')',
                        'align' => 'left'
                    );
                    $details_data_row[] = array(
                        'data' => $drow['serialnumber'],
    ...
    function detailed_payments($start_date, $end_date, $sale_type, $export_excel = 0, $offset = 0)
    {
        ...
        foreach ($report_data['summary'] as $sale_id => $row) {
            foreach ($row as $payment_type => $payment_data_row) {
                $summary_data_row = array();
                $summary_data_row[] = array(
                    'data' => anchor('sales/receipt/' . $payment_data_row['sale_id'], '<i class="ion-printer"></i>', array(
                        'target' => '_blank',
                        'class' => 'hidden-print'
                    )) . '<span class="visible-print">' . $payment_data_row['sale_id'] . '</span>' . anchor('sales/edit/' . $payment_data_row['sale_id'], '<i class="ion-document-text"></i>', array(
                        'target' => '_blank'
                    )) . ' ' . anchor('sales/edit/' . $payment_data_row['sale_id'], lang('common_edit') . ' ' . $payment_data_row['sale_id'], array(
                        'target' => '_blank',
                        'class' => 'hidden-print'
                    )),
                    'align' => 'left',
                    'detail_id' => $payment_data_row['sale_id']
                );
                $summary_data_row[] = array(
                    'data' => date(get_date_format() . '-' . get_time_format(), strtotime($payment_data_row['sale_time'])),
                    'align' => 'left'
                );
                $summary_data_row[] = array(
                    'data' => date(get_date_format() . '-' . get_time_format(), strtotime($payment_data_row['payment_date'])),
                    'align' => 'left'
                );
                $summary_data_row[] = array(
                    'data' => $payment_data_row['payment_type'],
                    'align' => 'left'
                );
                $summary_data_row[] = array(
                    'data' => to_currency($payment_data_row['payment_amount']),
                    'align' => 'right'
                );
        ...
        foreach ($report_data['details']['sale_ids'] as $sale_id => $drows) {
            ...
            foreach ($drows as $drow) {
                $details_data_row = array();
                $details_data_row[] = array(
                    'data' => date(get_date_format() . '-' . get_time_format(), strtotime($drow['payment_date'])),
                    'align' => 'left'
                );
                $details_data_row[] = array(
                    'data' => $drow['payment_type'],
                    'align' => 'left'
                );
                $details_data_row[] = array(
                    'data' => to_currency($drow['payment_amount']),
                    'align' => 'right'
                );
    ...
    function detailed_suspended_sales($start_date, $end_date, $sale_type, $export_excel = 0, $offset = 0)
    {
        ...
        foreach (isset($export_excel) == 1 && isset($report_data['summary']) ? $report_data['summary'] : $report_data as $key => $row) {
            ...
            if ($row['suspended'] == 1) {
                $summary_data_row[] = array(
                    'data' => ($this->config->item('user_configured_layaway_name') ? $this->config->item('user_configured_layaway_name') : lang('common_layaway')),
                    'align' => 'right'
                );
            } elseif ($row['suspended'] == 2) {
                $summary_data_row[] = array(
                    'data' => lang('common_estimate'),
                    'align' => 'right'
                );
            } elseif ($row['was_layaway'] == 1) {
                $summary_data_row[] = array(
                    'data' => lang('reports_completed_layaway'),
                    'align' => 'right'
                );
            } elseif ($row['was_estimate'] == 1) {
                $summary_data_row[] = array(
                    'data' => lang('reports_completed_estimate'),
                    'align' => 'right'
                );
            }
        ...
        foreach (isset($export_excel) == 1 && isset($report_data['summary']) ? $report_data['summary'] : $report_data as $key => $row) {
            ...
            if ($export_excel == 1) {
                foreach ($report_data['details'][$key] as $drow) {
                    $details_data_row = array();
                    $details_data_row[] = array(
                        'data' => isset($drow['item_number']) ? $drow['item_number'] : $drow['item_number'],
                        'align' => 'left'
                    );
                    $details_data_row[] = array(
                        'data' => isset($drow['item_product_id']) ? $drow['item_product_id'] : $drow['item_product_id'],
                        'align' => 'left'
                    );
                    $details_data_row[] = array(
                        'data' => isset($drow['item_name']) ? $drow['item_name'] : $drow['item_kit_name'],
    ...
    function specific_supplier_receivings($start_date, $end_date, $supplier_id, $sale_type, $export_excel = 0, $offset = 0)
    {
        ...
        foreach (isset($export_excel) == 1 && isset($report_data['summary']) ? $report_data['summary'] : $report_data as $key => $row) {
            ...
            $summary_data_row[] = array(
                'data' => anchor('receivings/edit/' . $row['receiving_id'], lang('common_edit') . ' ' . $row['receiving_id'], array(
                    'target' => '_blank'
                )) . ' [' . anchor('items/generate_barcodes_from_recv/' . $row['receiving_id'], lang('common_barcode_sheet'), array(
                    'target' => '_blank'
                )) . ' / ' . anchor('items/generate_barcodes_labels_from_recv/' . $row['receiving_id'], lang('common_barcode_labels'), array(
                    'target' => '_blank'
                )) . ']',
                'align' => 'left',
                'detail_id' => $row['receiving_id']
            );
            ...
            $summary_data_row[] = array(
                'data' => date(get_date_format() . '-' . get_time_format(), strtotime($row['receiving_time'])),
                'align' => 'left'
            );
            $summary_data_row[] = array(
                'data' => to_quantity($row['items_purchased']),
            ...
            $summary_data_row[] = array(
                'data' => to_quantity($row['items_received']),
                'align' => 'left'
            );
            $summary_data_row[] = array(
                'data' => to_currency($row['subtotal']),
    ...
    function detailed_suspended_receivings($start_date, $end_date, $supplier_id, $sale_type, $export_excel = 0, $offset = 0)
    {
        ...
        foreach (isset($export_excel) == 1 && isset($report_data['summary']) ? $report_data['summary'] : $report_data as $key => $row) 
        {
            $summary_data[$key] = array(
                array(
                    'data' => anchor('receivings/receipt/' . $row['receiving_id'], '<i class="ion-printer"></i>', array(
                        'target' => '_blank'
                    )) . ' ' . anchor('receivings/edit/' . $row['receiving_id'], '<i class="ion-document-text"></i>', array(
                        'target' => '_blank'
                    )) . ' ' . anchor('receivings/edit/' . $row['receiving_id'], lang('common_edit') . ' ' . $row['receiving_id'], array(
                        'target' => '_blank'
                    )) . ' [' . anchor('items/generate_barcodes_from_recv/' . $row['receiving_id'], lang('common_barcode_sheet'), array(
                        'target' => '_blank'
                    )) . ' / ' . anchor('items/generate_barcodes_labels_from_recv/' . $row['receiving_id'], lang('common_barcode_labels'), array(
                ...
                array(
                    'data' => date(get_date_format(), strtotime($row['receiving_date'])),
                    'align' => 'left'
                ),
                array(
                    'data' => to_quantity($row['items_purchased']),
            ...
            if ($location_count > 1) {
                array_unshift($summary_data[$key], array(
                    'data' => $row['location_name'],
                    'align' => 'left'
                ));
            }
            ...
            $summary_data[$key] = array(
                array(
                    'data' => anchor('receivings/receipt/' . $row['receiving_id'], '<i class="ion-printer"></i>', array(
                        ...
                        'target' => '_blank',
                        'class' => 'generate_barcodes_from_recv'
                    )) . ' / ' . anchor('items/generate_barcodes_labels_from_recv/' . $row['receiving_id'], lang('common_barcode_labels'), array(
                        'target' => '_blank'
                    )) . ']' . $transfer_info,
                    'align' => 'left',
    ...
    function inventory_low($supplier = -1, $category_id = -1, $inventory = 'all', $reorder_only = 0, $export_excel = 0, $offset = 0)
    {
        ...
        foreach ($report_data as $row) {
            ...
            $data_row[] = array(
                'data' => to_quantity($row['reorder_level']),
                'align' => 'left'
            );
    ...
    function inventory_summary($inventory_date, $supplier = -1, $category_id = -1, $inventory = 'all', $show_only_pending = 0, $export_excel = 0, $offset = 0)
    {
        $category_id = rawurldecode($category_id);
        ...
        $model->setParams(array(
            'inventory_date' => $inventory_date . ' 23:59:59',
            'supplier' => $supplier,
            'category_id' => $category_id,
            'export_excel' => $export_excel,
            'offset' => $offset,
        ...
        $config = array();
        $config['base_url'] = site_url("reports/inventory_summary/$inventory_date/$supplier/$category_id/$inventory/$show_only_pending/$export_excel");
        $config['total_rows'] = $model->getTotalRows();
        $config['per_page'] = $this->config->item('number_of_items_per_page') ? (int) $this->config->item('number_of_items_per_page') : 20;
        $config['uri_segment'] = 9;
        $this->load->library('pagination');
        ...
        foreach ($report_data as $row) {
            ...
            $data_row[] = array(
                'data' => to_quantity($row['pending_inventory']),
                'align' => 'left'
            );
            $data_row[] = array(
                'data' => to_quantity($row['reorder_level']),
    ...
    function summary_giftcard_sales($start_date, $end_date, $sale_type, $export_excel = 0, $offset = 0)
    {
        ...
        foreach ($report_data as $row) {
            ...
            $data_row[] = array(
                'data' => date(get_date_format(), strtotime($row['sale_time'])),
                'align' => 'left'
            );
            $data_row[] = array(
                'data' => $row['giftcard_number'],
            ...
            $data_row[] = array(
                'data' => to_currency($row['gift_card_sale_price']),
                'align' => 'left'
            );
    ...
    function summary_store_accounts($show_accounts_over_credit_limit, $export_excel = 0, $offset = 0)
    {
        ...
        $model->setParams(array(
            'show_accounts_over_credit_limit' => $show_accounts_over_credit_limit,
            'export_excel' => $export_excel,
            'offset' => $offset
        ));
        ...
        foreach ($report_data as $row) {
            $tabular_data[] = array(
                ...
                array(
                    'data' => anchor("customers/pay_now/" . $row['person_id'], lang('common_pay'), array(
                        'title' => lang('common_update'),
                        'class' => 'btn btn-info'
                    )),
                    'align' => 'right'
                )
        ...
        foreach (isset($export_excel) == 1 && isset($report_data['summary']) ? $report_data['summary'] : $report_data as $key => $row) {
            ...
            if ($export_excel == 1) {
                foreach ($report_data['details'][$key] as $drow) {
                    ...
                    $details_data_row[] = array(
                        'data' => isset($drow['item_number']) ? $drow['item_number'] : $drow['item_kit_number'],
                        'align' => 'left'
                    );
                    $details_data_row[] = array(
                        'data' => isset($drow['item_name']) ? $drow['item_name'] : $drow['item_kit_name'],
    ...
    function detailed_inventory($start_date, $end_date, $item_id, $show_manual_adjustments_only, $export_excel = 0, $offset = 0)
    {
        ...
        $model->setParams(array(
            ...
            'end_date' => $end_date,
            'item_id' => $item_id,
            'show_manual_adjustments_only' => $show_manual_adjustments_only,
            'offset' => $offset,
            'export_excel' => $export_excel
        ));
        ...
        foreach ($report_data as $row) {
            ...
            $tabular_data_row = array(
                ...
                array(
                    'data' => date(get_date_format() . '-' . get_time_format(), strtotime($row['trans_date'])),
                    'align' => 'left'
                ),
                array(
                    'data' => $row['name'],
                ...
                array(
                    'data' => to_quantity($row['trans_inventory']),
                    'align' => 'left'
                ),
                array(
                    'data' => $row['trans_comment'],
                    'align' => 'left'
                )
            );
    ...
    function summary_commissions($start_date, $end_date, $sale_type, $employee_type, $export_excel = 0, $offset = 0)
    {
        ...
        foreach ($report_data as $row) {
            ...
            $data_row[] = array(
                'data' => to_currency($row['commission']),
                'align' => 'right'
            );
            $tabular_data[] = $data_row;
        }
        ...
        foreach (isset($export_excel) == 1 && isset($report_data['summary']) ? $report_data['summary'] : $report_data as $key => $row) {
            ...
            if ($export_excel == 1) {
                ...
                foreach ($report_data['details'][$key] as $drow) {
                    ...
                    $details_data_row[] = array(
                        'data' => to_currency($drow['commission']),
                        'align' => 'right'
                    );
    ...
    function detailed_timeclock($start_date, $end_date, $employee_id, $export_excel = 0, $offset = 0)
    {
        ...
        foreach ($report_data as $row) {
            ...
            $data_row[] = array(
                'data' => date(get_date_format() . ' ' . get_time_format(), strtotime($row['clock_in'])),
                'align' => 'left'
            );
            
            if ($row['clock_out'] != '0000-00-00 00:00:00') {
                $data_row[] = array(
                    'data' => date(get_date_format() . ' ' . get_time_format(), strtotime($row['clock_out'])),
                    'align' => 'left'
                );
                ...
                $data_row[] = array(
                    'data' => lang('reports_not_clocked_out'),
                    'align' => 'left'
                );
            ...
            $data_row[] = array(
                'data' => to_currency($row['hourly_pay_rate']),
                'align' => 'left'
            );
            $data_row[] = array(
                'data' => to_currency($row['hourly_pay_rate'] * $hours),
                'align' => 'left'
            );
            $data_row[] = array(
                'data' => $row['clock_in_comment'],
                'align' => 'left'
            );
            $data_row[] = array(
                'data' => $row['clock_out_comment'],
                'align' => 'left'
            );
            $data_row[] = array(
                'data' => $row['ip_address_clock_in'],
                'align' => 'left'
            );
            $data_row[] = array(
                'data' => $row['ip_address_clock_out'],
                'align' => 'left'
            );
    ...
    function closeout($start_date, $end_date, $export_excel = 0)
    {
        ...
        $start_date = date('Y-m-d 00:00:00', strtotime(rawurldecode($start_date)));
        $end_date = date('Y-m-d 23:59:59', strtotime(rawurldecode($end_date)));
        ...
        $model->setParams(array(
            'start_date' => $start_date,
            'end_date' => $end_date,
            'sales_total_for_payments' => $sales_totals_for_payments,
            'export_excel' => $export_excel
        ));
    ...
    function summary_tags($start_date, $end_date, $do_compare, $compare_start_date, $compare_end_date, $sale_type, $export_excel = 0, $offset = 0)
    {
        ...
        if ($do_compare) {
            ...
            $model_compare->setParams(array(
                ...
                'offset' => $offset,
                'export_excel' => $export_excel,
                'compare_to_tags' => $compare_to_tags
            ));
    ...
    function expiring_inventory($start_date, $end_date, $export_excel = 0, $offset = 0)
    {
        ...
        foreach ($report_data as $row) {
            ...
            $data_row[] = array(
                'data' => date(get_date_format(), strtotime($row['expire_date'])),
                'align' => 'left'
            );
            $data_row[] = array(
                'data' => to_quantity($row['quantity_expiring']),
                'align' => 'left'
            );
            $data_row[] = array(
                'data' => $this->Category->get_full_path($row['category_id']),
    ...
    function detailed_expenses($start_date, $end_date, $export_excel = 0, $offset = 0)
    {
        ...
        foreach ($report_data as $row) {
            $tabular_data_row = array(
                ...
                array(
                    'data' => $row['expense_type'],
                    'align' => 'left'
                ),
                array(
                    'data' => $row['expense_description'],
                    'align' => 'left'
                ),
                array(
                    'data' => $this->Category->get_full_path($row['category_id']),
                ...
                array(
                    'data' => $row['expense_reason'],
                    'align' => 'left'
                ),
                array(
                    'data' => date(get_date_format(), strtotime($row['expense_date'])),
                    'align' => 'left'
                ),
                array(
                    'data' => to_currency($row['expense_amount']),
                    'align' => 'left'
                ),
                array(
                    'data' => to_currency($row['expense_tax']),
                    'align' => 'left'
                ),
                array(
                    'data' => $row['employee_recv'],
                    'align' => 'left'
                ),
                array(
                    'data' => $row['employee_appr'],
                    'align' => 'left'
                ),
                array(
                    'data' => $row['expense_note'],
                    'align' => 'left'
                )
            );
    ...
    function giftcard_audit($start_date, $end_date, $giftcard_number = -1, $export_excel = 0, $offset = 0)
    {
        ...
        foreach ($report_data as $row) {
            ...
            $tabular_data[] = array(
                array(
                    'data' => date(get_date_format() . ' ' . get_time_format(), strtotime($row['log_date'])),
                    'align' => 'left'
                ),
                array(
                    'data' => $row['giftcard_number'],
    ...
    function store_account_outstanding($customer_id = -1, $show_paid = 0, $export_excel = 0, $offset = 0)
    {
        ...
        foreach ($report_data as $row) {
            ...
            $tabular_data[] = array(
                array(
                    'data' => anchor('sales/receipt/' . $row['sale_id'], ($this->config->item('sale_prefix') ? $this->config->item('sale_prefix') : 'POS') . ' ' . $row['sale_id'], array(
                        'target' => '_blank'
                    )),
                    'align' => 'left'
                ),
                ...
                array(
                    'data' => to_currency($row['payment_amount']),
                    'align' => 'left'
                ),
                array(
                    'data' => $row['comment'],
                ...
                array(
                    'data' => $mark_paid_unpaid,
                    'align' => 'center'
                )
            );
        }
        
        $mark_all_paid = anchor('reports/store_account_outstanding_mark_all_as_paid/' . $customer_id . '/' . $show_paid . '/' . $offset, lang('reports_mark_all_as_paid'), "onclick='return do_link_confirm(" . json_encode(lang('reports_confirm_all_mark_as_paid')) . ", this)'");
    ...
    {
        $this->check_action_permission('view_store_account');
        $this->db->insert('store_accounts_paid_sales', array(
            'sale_id' => $sale_id,
            'store_account_payment_sale_id' => NULL
        ));
    ...
    {
        $this->check_action_permission('view_store_account');
        $this->db->delete('store_accounts_paid_sales', array(
            'sale_id' => $sale_id
        ));
        redirect('reports/store_account_outstanding/' . $customer_id . '/' . $show_paid . '/0/' . $offset);
    }
    ...
    function supplier_summary_store_accounts($export_excel = 0, $offset = 0)
    {
        ...
        foreach ($report_data as $row) {
            $tabular_data[] = array(
                ...
                array(
                    'data' => anchor("suppliers/pay_now/" . $row['person_id'], lang('common_pay'), array(
                        'title' => lang('common_update'),
                        'class' => 'btn btn-info'
                    )),
                    'align' => 'right'
    ...
    function supplier_specific_store_account($start_date, $end_date, $supplier_id, $receiving_type, $export_excel = 0, $offset = 0)
    {
        ...
        foreach ($report_data as $row) {
            $tabular_data[] = array(
                ...
                array(
                    'data' => $row['receiving_id'] ? anchor('receivings/receipt/' . $row['receiving_id'], 'RECV ' . $row['receiving_id'], array(
                        'target' => '_blank'
                    )) : '-',
                    'align' => 'center'
                ),
    ...
    function supplier_store_account_activity($start_date, $end_date, $export_excel = 0, $offset = 0)
    {
        ...
        foreach ($report_data as $row) {
            $tabular_data[] = array(
                ...
                array(
                    'data' => $row['company_name'] . ' (' . $row['first_name'] . ' ' . $row['last_name'] . ')',
                    'align' => 'left'
                ),
                array(
                    'data' => date(get_date_format() . '-' . get_time_format(), strtotime($row['date'])),
    ...
    function supplier_store_account_outstanding($supplier_id = -1, $show_paid = 0, $export_excel = 0, $offset = 0)
    {
        ...
        foreach ($report_data as $row) {
            ...
            $tabular_data[] = array(
                array(
                    'data' => anchor('receivings/receipt/' . $row['receiving_id'], 'RECV ' . $row['receiving_id'], array(
                        'target' => '_blank'
                    )),
                    'align' => 'left'
                ),
                array(
                    'data' => $row['supplier_name'] . (isset($row['account_number']) && $row['account_number'] ? ' (' . $row['account_number'] . ')' : ''),
                    'align' => 'left'
                ),
                array(
                    'data' => date(get_date_format() . ' ' . get_time_format(), strtotime($row['receiving_time'])),
        ...
        $mark_all_paid = anchor('reports/supplier_store_account_outstanding_mark_all_as_paid/' . $supplier_id . '/' . $show_paid . '/' . $offset, lang('reports_mark_all_as_paid'), "onclick='return do_link_confirm(" . json_encode(lang('reports_confirm_all_mark_as_paid')) . ", this)'");
    ...
    {
        $this->check_action_permission('view_store_account_suppliers');
        $this->db->insert('supplier_store_accounts_paid_receivings', array(
            'receiving_id' => $receiving_id,
            'store_account_payment_receiving_id' => NULL
        ));
    ...
    {
        $this->check_action_permission('view_store_account_suppliers');
        $this->db->delete('supplier_store_accounts_paid_receivings', array(
            'receiving_id' => $receiving_id
        ));
        redirect('reports/supplier_store_account_outstanding/' . $supplier_id . '/' . $show_paid . '/0/' . $offset);
    }
    ...
    function receivings_detailed_payments($start_date, $end_date, $receiving_type, $export_excel = 0, $offset = 0)
    {
        ...
        foreach ($report_data['summary'] as $receiving_id => $row) {
            foreach ($row as $payment_type => $payment_data_row) {
                ...
                $summary_data_row[] = array(
                    'data' => anchor('receivings/receipt/' . $payment_data_row['receiving_id'], '<i class="ion-printer"></i>', array(
                        'target' => '_blank',
                        'class' => 'hidden-print'
                    )) . '<span class="visible-print">' . $payment_data_row['receiving_id'] . '</span>' . anchor('receivings/edit/' . $payment_data_row['receiving_id'], '<i class="ion-document-text"></i>', array(
                        'target' => '_blank'
                    )) . ' ' . anchor('receivings/edit/' . $payment_data_row['receiving_id'], lang('common_edit') . ' ' . $payment_data_row['receiving_id'], array(
                        'target' => '_blank',
                        'class' => 'hidden-print'
                    )),
                    'align' => 'left',
                    'detail_id' => $payment_data_row['receiving_id']
                );
                $summary_data_row[] = array(
                    'data' => date(get_date_format() . '-' . get_time_format(), strtotime($payment_data_row['receiving_time'])),
                    'align' => 'left'
                );
                $summary_data_row[] = array(
                    'data' => date(get_date_format() . '-' . get_time_format(), strtotime($payment_data_row['payment_date'])),
    ...
    function summary_items_variance($start_date, $end_date, $sale_type, $export_excel = 0, $offset = 0)
    {
        ...
        foreach ($report_data as $row) {
            ...
            $data_row[] = array(
                'data' => to_quantity($row['quantity_purchased']),
                'align' => 'left'
            );
            $data_row[] = array(
                'data' => to_currency($row['current_selling_price']),
            ...
            $data_row[] = array(
                'data' => '<span class="compare ' . ($row['variance_from_sale_price'] >= 0 ? ($row['variance_from_sale_price'] == 0 ? '' : 'compare_better') : 'compare_worse') . '">' . to_currency($row['variance_from_sale_price']) . '</span>',
                'align' => 'right'
            );
            $data_row[] = array(
                'data' => to_currency($row['total']),
    ...
    function summary_manufacturers($start_date, $end_date, $do_compare, $compare_start_date, $compare_end_date, $sale_type, $export_excel = 0, $offset = 0)
    {
        ...
        if ($do_compare) {
            ...
            $model_compare->setParams(array(
                ...
                'offset' => $offset,
                'export_excel' => $export_excel,
                'compare_to_manufacturers' => $compare_to_manufacturers
            ));
