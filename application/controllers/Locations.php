...
class Locations extends Secure_area implements Idata_controller
{
    ...
    {
        $params = $this->session->userdata('location_search_data') ? $this->session->userdata('location_search_data') : array(
            'offset' => 0,
            'order_col' => 'location_id',
            'order_dir' => 'asc',
            'search' => FALSE
        );
    ...
    function view($location_id = -1, $redirect = false)
    {
        ...
        foreach ($this->Employee->get_all()->result() as $employee) {
            ...
            $data['employees'][$employee->person_id] = array(
        ...
        if (! is_on_demo_host()) {
            if ($location_id) {
                ...
                if ($register_0_terminal_id = $this->input->post('terminal_id')) {
                    ...
                    $register_data = array(
                        'emv_terminal_id' => $register_0_terminal_id
                    );
    ...
    function save($location_id = -1)
    {
        ...
        } else // failure
        {
            echo json_encode(array(
                'success' => false,
                'message' => lang('locations_error_adding_updating')
            ));
        }
    }
        ...
        if ($registers_to_edit) {
            foreach ($registers_to_edit as $register_id => $register) {
                if ($register['name']) {
                    $register_data = array(
                        'name' => $register['name'],
                        'iptran_device_id' => $register['iptran_device_id'],
                        'emv_terminal_id' => $register['emv_terminal_id'],
                        'location_id' => $location_id
                    );
    ...
    function delete()
    {
        ...
        if ($this->Location->delete_list($locations_to_delete)) {
            ...
            echo json_encode(array(
                'success' => true,
                'message' => lang('locations_successful_deleted') . ' ' . lang('locations_one_or_multiple')
            ));
        } else {
            echo json_encode(array(
                'success' => false,
                'message' => lang('locations_cannot_be_deleted')
            ));
        }
    }
}
