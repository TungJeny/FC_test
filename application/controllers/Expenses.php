...
class Expenses extends Secure_area implements Idata_controller
{
    ...
    {
        $params = $this->session->userdata('expenses_search_data') ? $this->session->userdata('expenses_search_data') : array(
            'offset' => 0,
            'order_col' => 'id',
            'order_dir' => 'desc',
            'search' => FALSE
    ...
    function save($id = -1)
    {
        ...
        if ($this->Expense->save($expense_data, $id)) {
            ...
            echo json_encode(array(
                'success' => false,
                'message' => lang('expenses_error_adding_updating')
            ));
        }
    }
    ...
    function delete()
    {
        ...
        if ($this->Expense->delete_list($expenses_to_delete)) {
            ...
            echo json_encode(array(
                'success' => true,
                'message' => lang('expenses_successful_deleted') . ' ' . lang('expenses_one_or_multiple')
            ));
        } else {
            echo json_encode(array(
                'success' => false,
                'message' => lang('expenses_cannot_be_deleted')
            ));
        }
    }
}
