...
class Items extends Secure_area implements Idata_controller
{
     ...
     */
    function view_type_unit_item() {
        $total_quality = $this->uri->segment(3);
        $item_id= $this->uri->segment(4);
        $this->load->model('Unit');
        $this->load->model('Item');
        $this->load->helper('unit');
        $data['type_unit_item'] = get_unit_text($item_id, $total_quality);
        $this->load->view('items/modal_type_unit', $data);
    }

    function generate_qrcode_labels($item_ids){
        $this->load->model('appconfig');
        $data['website'] = $this->appconfig->get('website');
        $data['company'] = $this->appconfig->get('company');
        $this->load->helper('qrcode');
        $data['items'] = get_items_qrcode_data($item_ids);
        $this->load->view('qrcode_labels', $data);
    }
    ...
    function categories()
    {
        ...
        $data = array(
            'category_tree' => $this->_category_tree_list($categories)
        );
    ...
    function save_category($category_id = FALSE)
    {
        ...
        if ($this->Category->save($category_name, $hide_from_grid, $parent_id, $category_id, $category_color, $category_image_id)) {
            echo json_encode(array(
                'success' => true,
                'message' => lang('items_category_successful_adding') . ' ' . $category_name
            ));
        } else {
            echo json_encode(array(
                'success' => false,
                'message' => lang('items_category_successful_error')
            ));
        }
    }
    ...
    function delete_category()
    {
        ...
        if ($this->Category->delete($category_id)) {
            echo json_encode(array(
                'success' => true,
                'message' => lang('items_successful_deleted')
            ));
        } else {
            echo json_encode(array(
                'success' => false,
                'message' => lang('items_cannot_be_deleted')
            ));
        }
    }
    ...
    function manage_units()
    {
        // $this->check_action_permission('manage_units');
        $units = $this->Unit->get_all();
        $data = array(
            'units' => $units,
            'unit_list' => $this->_unit_list()
        );
        $this->load->view('items/units', $data);
    }
    
    function save_unit($unit_id = FALSE)
    {
        // $this->check_action_permission('manage_units');
        $unit_name = $this->input->post('unit_name');
        if ($this->Unit->save($unit_name, $unit_id)) {
            echo json_encode(array(
                'success' => true,
                'message' => lang('items_unit_successful_adding') . ' ' . $unit_name
            ));
        } else {
            echo json_encode(array(
                'success' => false,
                'message' => lang('items_unit_successful_error')
            ));
        }
    }
    ...
    function _unit_list()
    {
        ...
        foreach ($units as $unit_id => $unit) {
            $return .= '<li>' . $unit['name'] . '<a href="javascript:void(0);" class="edit_unit" data-name = "' . H($unit['name']) . '" data-unit_id="' . $unit_id . '">[' . lang('common_edit') . ']</a> ' . '<a href="javascript:void(0);" class="delete_unit" data-unit_id="' . $unit_id . '">[' . lang('common_delete') . ']</a> ';
            $return .= '</li>';
        }
        $return .= '</ul>';
    ...
    function save_tag($tag_id = FALSE)
    {
        ...
        if ($this->Tag->save($tag_name, $tag_id)) {
            echo json_encode(array(
                'success' => true,
                'message' => lang('items_tag_successful_adding') . ' ' . $tag_name
            ));
        } else {
            echo json_encode(array(
                'success' => false,
                'message' => lang('items_tag_successful_error')
            ));
        }
    }
    ...
    function _tag_list()
    {
        ...
        foreach ($tags as $tag_id => $tag) {
            $return .= '<li>' . $tag['name'] . '<a href="javascript:void(0);" class="edit_tag" data-name = "' . H($tag['name']) . '" data-tag_id="' . $tag_id . '">[' . lang('common_edit') . ']</a> ' . '<a href="javascript:void(0);" class="delete_tag" data-tag_id="' . $tag_id . '">[' . lang('common_delete') . ']</a> ';
            $return .= '</li>';
        }
        $return .= '</ul>';
    ...
    function _category_tree_list($tree)
    {
        ...
        if (! is_null($tree) && count($tree) > 0) {
            ...
            foreach ($tree as $node) {
                $return .= '<li>' . $node->name . ' <a href="javascript:void(0);" class="add_child_category" data-category_id="' . $node->id . '">[' . lang('items_add_child_category') . ']</a> ' . '<a href="javascript:void(0);" class="edit_category" data-color="' . H($node->color) . '" data-image_id="' . H($node->image_id) . '" data-image_timestamp="' . $this->Appfile->get_file_timestamp($node->image_id) . '" data-name = "' . H($node->name) . '" data-parent_id = "' . $node->parent_id . '" data-category_id="' . $node->id . '">[' . lang('common_edit') . ']</a> ' . '<a href="javascript:void(0);" class="delete_category" data-category_id="' . $node->id . '">[' . lang('common_delete') . ']</a> ' . '&nbsp;&nbsp;&nbsp;<label for="hide_from_grid_' . $node->id . '">' . lang('items_hide_from_item_grid') . '</label> <input type="checkbox" ' . ($node->hide_from_grid ? 'checked="checked"' : '') . ' class="hide_from_grid" id="hide_from_grid_' . $node->id . '" value="1" name="hide_from_grid_' . $node->id . '" data-category_id="' . $node->id . '" /> <label for="hide_from_grid_' . $node->id . '"><span></span></label>';
    ...
    function manage_manufacturers()
    {
        ...
        $data = array(
            ...
            'manufacturers_list' => $this->_manufacturers_list()
        );
    ...
    function save_manufacturer($manufacturer_id = FALSE)
    {
        ...
        if ($this->Manufacturer->save($manufacturer_name, $manufacturer_id)) {
            echo json_encode(array(
                'success' => true,
                'message' => lang('items_manufacturer_successful_adding') . ' ' . $manufacturer_name
            ));
        } else {
            echo json_encode(array(
                'success' => false,
                'message' => lang('items_manufacturer_successful_error')
            ));
        }
    }
    ...
    function _manufacturers_list()
    {
        ...
        foreach ($manufacturers as $manufacturers_id => $manufacturers) {
            $return .= '<li>' . $manufacturers['name'] . '<a href="javascript:void(0);" class="edit_manufacturer" data-name = "' . H($manufacturers['name']) . '" data-manufacturer_id="' . $manufacturers_id . '">[' . lang('common_edit') . ']</a> ' . '<a href="javascript:void(0);" class="delete_manufacturer" data-manufacturer_id="' . $manufacturers_id . '">[' . lang('common_delete') . ']</a> ';
            $return .= '</li>';
        }
        $return .= '</ul>';
    ...
    {
        echo json_encode(array(
            'duplicate' => $this->Item->check_duplicate($this->input->post('term'))
        ));
    }
    ...
    function suggest_autocomplete()
    {
        ...
        $suggestions = $this->Item->search_suggestions($this->input->get('term'), $category_id, 100);
        echo json_encode($suggestions);
    }
    ...
    function _get_item_data($item_id)
    {
        ...
        $data['attr_groups'] = ['-1' => lang('common_none')];
        $attr_groups = $this->Attribute_group->getAll(['related_object' => 'item']);
        foreach ($attr_groups as $attr_group) {
            $data['attr_groups'][$attr_group['id']] = $attr_group['name'];
        }
        ...
        $units = ['-1' => lang('common_none')];
        foreach ($this->Unit->get_all() as $id => $row) {
            $units[$id] = $row['name'];
        }
        $data['units'] = $units;
        $data['selected_unit'] = $this->Item->get_info($item_id)->unit_id;
        ...
        foreach ($this->Supplier->get_all()->result_array() as $row) {
            ...
            $suppliers_formatted[] = ['person_id' => $row['person_id'], 'label' => $row['company_name']];
        }
        ...
        foreach ($this->Customer->get_all()->result_array() as $row) {
            $customers[$row['person_id']] = $row['code'] . ' (' . $row['first_name'] . ' ' . $row['last_name'] . ')';
            $customers_formatted[] = ['person_id' => $row['person_id'], 'label' => $row['code']];
        }

        // end query customer
        
        $data['tier_prices'] = array();
        $data['tier_type_options'] = array(
            'unit_price' => lang('common_fixed_price'),
        ...
        $data['suppliers_formatted'] = $suppliers_formatted;
        $data['selected_supplier'] = $this->Item->get_info($item_id)->supplier_id;
        $data['selected_suppliers'] = implode(',', $this->Supplier->get_suppliers_for_item($item_id));
        // add customers
        $data['customers'] = $customers;
        $data['customers_formatted'] = $customers_formatted;
        $data['selected_customer'] = $this->Item->get_info($item_id)->supplier_id;
        $data['selected_customers'] = implode(',', $this->Customer->get_customers_for_item_task($item_id));
        // end customers
        ...
        $data['vueObjects'] = [
            ...
            'attr_values' => $this->Attribute_value->get_all_values($item_id, 'item'),
            'attr_group_id' => $data['item_info']->attr_group_id
        ];
        return $data;
    }
    ...
    function save($item_id = -1)
    {
        ...
        //    if ($this->input->post('name') == $this->Item->get_item_by_product_id_or_name($this->input->post('name'))) {
        //        $item_data['name'] = $this->input->post('name');
        //    }else{
        //         $message = "sản phẩm này đã tồn tại bản ghi giống rùi";
        //    }
        // }
        ...
        $item_data = array(
            ...
            'buffer_rate' => $this->input->post('buffer_rate') ? $this->input->post('buffer_rate') : 1,
            'attr_group_id' => $this->input->post('attr_group_id') ? $this->input->post('attr_group_id') : NULL,
            //   begin add coefficient  
            'coefficient' => $this->input->post('coefficient') ? $this->input->post('coefficient') : NULL

            // end coefficient  
        );

        // check unit ĐVT .base code json save data
        if ($this->input->post('unit_number') && $this->input->post('unit_change') ) {

            $var_unit_number = $this->input->post('unit_number');
            $var_unit_change = $this->input->post('unit_change');
            $input_unit_id = $this->input->post('unit_id');
            if ($var_unit_number[0] == '' || $var_unit_change[0] == '') {
             
                }else{
                //
                $unit_number =  $this->input->post('unit_number'); 
                $unit_change = $this->input->post('unit_change');

                $data_unit = array_combine($unit_change,$unit_number);
                $quantity_unit = $this->Item->get_one_location_item($item_id);

                $item_data['type_unit_formula'] = json_encode($data_unit);
                // $item_data['type_unit'] = (string)(type_unit($data_unit, $quantity_unit, $input_unit_id)); 
            }
        }
        ...
        if ($this->Item->save($item_data, $item_id)) {
            ...
            $this->Supplier->save_suppliers_for_item(isset($item_data['item_id']) ? $item_data['item_id'] : $item_id, $this->input->post('suppliers'));
            // save Customers
            $this->Customer->save_customers_for_item(isset($item_data['item_id']) ? $item_data['item_id'] : $item_id, $this->input->post('customers'));
            // end Customers
            ...
            if ($item_id == - 1) {
                ...
                echo json_encode(array(
                    ...
                    'redirect' => $redirect,
                    'sale_or_receiving' => $sale_or_receiving
                ));
                $item_id = $item_data['item_id'];
            } else // previous item
            {
            ...
            if ($this->input->post('locations')) {
                foreach ($this->input->post('locations') as $location_id => $item_location_data) {
                    $item_location_data['location'] = !empty($item_location_data['location']) ? $item_location_data['location'] : '';
                ...
                if ($item_data['is_ecommerce']) {
                    foreach ($this->Item->get_item_images(isset($item_data['item_id']) ? $item_data['item_id'] : $item_id) as $image) {
                        $ecom_item_data['images'][] = array(
                            ...
                            'ecommerce_image_id' => $image['ecommerce_image_id'],
                            'src' => app_file_url($image['image_id']),
                            'name' => isset($titles[$image['image_id']]) ? $titles[$image['image_id']] : '',
                            'alt' => isset($alt_texts[$image['image_id']]) ? $alt_texts[$image['image_id']] : ''
                        );
                    }
            ...
            // TODO
            $semi_item_ids = $this->input->post('item_semis');
            $semi_items = [];
            if (!empty($semi_item_ids)) {
                foreach ($semi_item_ids as $semi_id => $qty) {
                    $semi_items[] = [
            ...
            $this->Mrp_Item_Semi->save($item_id, $semi_items);
            ...
            if (!empty($attrs)) {
                ...
                $this->Attribute_value->delete_by_entity($item_id, 'item');
                $this->Attribute_value->save_batch($attributes);
            }
            
        } else // failure
        {
            echo "<script type='text/javascript'>alert('tests')</script>";die();
            echo json_encode(array(
                'success' => false,
                'message' => lang('common_error_adding_updating') . ' ' . $item_data['name'],
                'item_id' => - 1
            ));
        }
    ...
    function save_inventory($item_id = -1)
    {
        ...
        if ($this->Item_location->save_quantity($cur_item_location_info->quantity + $this->input->post('newquantity'), $item_id)) {
            echo json_encode(array(
                'success' => true,
                'message' => lang('common_items_successful_updating') . ' ' . $cur_item_info->name,
                'item_id' => $item_id
            ));
        } else // failure
        {
            echo json_encode(array(
                'success' => false,
                'message' => lang('common_error_adding_updating') . ' ' . $cur_item_info->name,
                'item_id' => - 1
            ));
        }
    }
    ...
    function bulk_update()
    {
        ...
        foreach ($_POST as $key => $value) {
            ...
            if ($key == 'supplier_id') {
                ...
                $this->Item_location->update_multiple(array(
                    'quantity' => $value
                ), $items_to_update, $select_inventory);
            ...
            } elseif ($value != '' and ! (in_array($key, array(
                'cost_price',
                'unit_price',
                'promo_price',
                'item_cost_price_method',
                'item_unit_price_method',
                'item_promo_price_method',
                'item_ids',
                'tax_names',
                'tax_percents',
                'tax_cumulatives',
        ...
        if (empty($item_data) || $this->Item->update_multiple($item_data, $items_to_update, $select_inventory)) {
            ...
            echo json_encode(array(
                'success' => true,
                'message' => lang('items_successful_bulk_edit')
            ));
        } else {
            echo json_encode(array(
                'success' => false,
                'message' => lang('items_error_updating_multiple')
            ));
        }
    ...
    function delete()
    {
        ...
        if ($this->Item->delete_list($items_to_delete, $select_inventory)) {
            echo json_encode(array(
                'success' => true,
                'message' => lang('items_successful_deleted') . ' ' . $total_rows . ' ' . lang('items_one_or_multiple')
            ));
        } else {
            echo json_encode(array(
                'success' => false,
    ...
    function excel()
    {
        ...
        array_to_spreadsheet(array(
            $header_row
        ), 'items_import.' . ($this->config->item('spreadsheet_format') == 'XLSX' ? 'xlsx' : 'csv'));
    }
    ...
    function excel_export()
    {
        ...
        if ($search || $category_id) {
            $data = $this->Item->search($search, $category_id, $this->Item->search_count_all($search, $category_id, 10000, $params['fields']), 0, $params['order_col'], $params['order_dir'], $params['fields'])
                ->result_object();
        } else {
            $data = $this->Item->get_all($this->Item->count_all())
                ->result_object();
        }
        ...
        foreach ($data as $r) {
            ...
            foreach ($tiers as $tier) {
                ...
                if (isset($tier_prices[$r->item_id][$tier->id])) {
                    ...
                    $value = $tier_prices[$r->item_id][$tier->id]['unit_price'] !== NULL ? to_currency_no_money($tier_prices[$r->item_id][$tier->id]['unit_price']) : $percent_value;
                }
    ...
    function do_excel_upload()
    {
        ...
        $app_file_file_id = $this->Appfile->save($_FILES["file"]["name"], file_get_contents($_FILES["file"]["tmp_name"]), date('Y-m-d H:i:s', strtotime("+3 hours")));
        ...
        foreach ($first_row as $col_name) {
            ...
            if ($column['Spreadsheet Column'] == '') {
                echo json_encode(array(
                    'success' => false,
                    'message' => lang('common_all_spreadsheet_columns_must_have_labels')
                ));
                return;
            }
    ...
    function do_excel_import_map()
    {
        ...
        while ($col_name = $sheet->getCellByColumnAndRow($k, 1)) {
            ...
            $col_data = array();
            for ($i = 2; $i <= $numRows; $i ++) {
                $col_data[] = trim($sheet->getCellByColumnAndRow($k, $i));
            }
    ...
    private function _get_database_fields_for_import_as_array()
    {
        ...
        $fields[] = array(
            'Name' => lang('common_item_number'),
            'key' => 'item_number'
        );
        $fields[] = array(
            'Name' => lang('common_product_id'),
            'key' => 'product_id'
        );
        $fields[] = array(
        ...
        $fields[] = array(
            'Name' => lang('common_category'),
            'key' => 'category_id'
        );
        $fields[] = array(
            'Name' => lang('common_supplier_id'),
            'key' => 'supplier_id'
        );
        $fields[] = array(
            'Name' => lang('common_cost_price'),
            'key' => 'cost_price'
        );
        $fields[] = array(
            'Name' => lang('common_unit_price'),
            'key' => 'unit_price'
        );
        $fields[] = array(
            'Name' => lang('items_promo_price'),
            'key' => 'promo_price'
        );
        $fields[] = array(
            'Name' => lang('items_promo_start_date'),
            'key' => 'start_date'
        );
        $fields[] = array(
            'Name' => lang('items_promo_end_date'),
            'key' => 'end_date'
        );
        ...
        $fields[] = array(
            'Name' => lang('items_price_includes_tax'),
            'key' => 'tax_included'
        );
        $fields[] = array(
            'Name' => lang('items_is_service'),
            'key' => 'is_service'
        );
        $fields[] = array(
            'Name' => lang('items_quantity'),
            'key' => 'quantity'
        );
        $fields[] = array(
            'Name' => lang('items_reorder_level'),
            'key' => 'reorder_level'
        );
        $fields[] = array(
            'Name' => lang('common_description'),
            'key' => 'description'
        );
        $fields[] = array(
            'Name' => lang('items_allow_alt_desciption'),
            'key' => 'allow_alt_description'
        );
        $fields[] = array(
            'Name' => lang('items_is_serialized'),
            'key' => 'is_serialized'
        );
        $fields[] = array(
            'Name' => lang('common_size'),
            'key' => 'size'
        );
        $fields[] = array(
            'Name' => lang('reports_commission'),
            'key' => 'commission'
        );
        $fields[] = array(
            'Name' => lang('items_commission_percent_based_on_profit'),
            'key' => 'commission_percent_type'
        );
        $fields[] = array(
            'Name' => lang('items_non_taxable'),
            'key' => 'override_default_tax'
        );
        $fields[] = array(
            'Name' => lang('common_tags'),
            'key' => 'tags'
        );
        $fields[] = array(
            'Name' => lang('items_days_to_expiration'),
            'key' => 'expire_days'
        );
        $fields[] = array(
            'Name' => lang('common_change_cost_price_during_sale'),
            'key' => 'change_cost_price'
        );
        $fields[] = array(
            'Name' => lang('common_manufacturer'),
            'key' => 'manufacturer_id'
        );
        $fields[] = array(
            'Name' => lang('items_location_at_store'),
            'key' => 'location'
        );
        if ($this->config->item('enable_customer_loyalty_system') && $this->config->item('loyalty_option') == 'advanced') {
            $fields[] = array(
                'Name' => lang('common_disable_loyalty'),
                'key' => 'disable_loyalty'
            );
        }
        ...
        if ($this->config->item("ecommerce_platform")) {
            $fields[] = array(
                'Name' => lang('items_is_ecommerce'),
                'key' => 'is_ecommerce'
            );
        }
        
        $fields[] = array(
            'Name' => lang('common_item_id'),
            'key' => 'item_id'
        );
    ...
    function dedup_excel_import_data()
    {
        ...
        $fieldId_to_colIndex = array_flip(array_map(array(
            $this,
            '_indexColumnArray'
        ), $columns_with_data));
        ...
        if (count($item_number_dups) > 0 || count($product_id_dups) > 0) {
            echo json_encode(array(
                'type' => 'error',
                'message' => lang('items_duplicate_item_numbers_product_ids'),
                'title' => lang('common_error')
            ));
        } else {
            echo json_encode(array(
                'type' => 'success',
                'message' => lang('items_no_duplicate_item_numbers_product_ids'),
    ...
    function complete_excel_import()
    {
        ...
        for ($i = 0; $i < $numRows - 1; $i ++) {
            ...
            if (isset($location_at_store)) {
                $this->Item_location->save(array(
                    'location' => $location_at_store,
                    'quantity' => NULL
            ...
            if ($item_id && isset($quantity) && ! empty($quantity) && ($quantity !== '0')) {
            ...
            $non_taxable = false;
            if (array_key_exists('override_default_tax', $item_data)) {
                $non_taxable = $item_data['override_default_tax'];
            }
        ...
        if ($key == 'supplier_id') {
            if ($value) {
                ...
                if (! $value) {
                    ...
                    $supplier_data = array(
                        'company_name' => $supplier_name_before_searching
                    );
    ...
    function cleanup()
    {
        ...
        echo json_encode(array(
            'success' => true,
            'message' => lang('items_cleanup_sucessful')
        ));
    }
    ...
    function do_count($count_id, $offset = 0)
    {
        ...
        $data['modes'] = array(
            'scan_and_set' => lang('items_scan_and_set'),
            'scan_and_add' => lang('items_scan_and_add')
        );
    ...
    {
        return array(
            lang('common_item_id') . '/' . lang('common_item_number') . '/' . lang('common_product_id'),
            lang('items_count')
        );
    }
    ...
    function excel_count()
    {
        ...
        array_to_spreadsheet(array(
            $header_row
        ), 'items_count.' . ($this->config->item('spreadsheet_format') == 'XLSX' ? 'xlsx' : 'csv'));
    }
    ...
    function do_excel_import_count()
    {
        ...
        if ($file_info['extension'] != 'xlsx' && $file_info['extension'] != 'csv') {
            echo json_encode(array(
                'success' => false,
                'message' => lang('common_upload_file_not_supported_format')
            ));
            return;
        }
    ...
    public function get_list()
    {
        ...
        foreach ($categories as $category) {
            $_categories = $this->Category->get_category_id_and_children_category_ids_for_category_id($category);
            if (!empty($_categories)) {
                $all_categories = array_merge($all_categories, $_categories);
            }
        }
        $options = [];
        $items = $this->Item->get_all_4_listview([
            'limit' => $limit,
            'offset' => $offset,
            'query' => $query,
            'order_by' => $orderBy,
        ...
        $totalRow = $this->Item->count_all_4_listview(['query' => $query, 'categories' => $all_categories]);
        echo json_encode([
            'type' => 'items',
            'data' => [
                'list' => $items,
