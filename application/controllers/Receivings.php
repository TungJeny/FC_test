...
class Receivings extends Secure_area
{
    ...
    function complete()
    {
        ...
        $old_date = $this->receiving_lib->get_change_recv_id() ? $this->Receiving->get_info($this->receiving_lib->get_change_recv_id())
            ->row_array() : false;
    ...
    function suspend()
    {
        ...
        if ($data['receiving_id'] == 'RECV -1') {
            $this->_reload(array(
                'error' => lang('receivings_transaction_failed')
            ));
            return;
        }
        ...
        if ($this->config->item('show_receipt_after_suspending_sale') || $is_po) {
            ...
            $this->receiving_lib->clear_all();
            $this->_reload(array(
                'success' => lang('receivings_successfully_suspended_receiving')
            ));
        }
    }
    ...
    function do_excel_import()
    {
                ...
                for ($k = 2; $k <= $num_rows; $k ++) {
                    ...
                    if ($this->receiving_lib->is_valid_item_kit($item_id)) {
                        if (! $this->receiving_lib->add_item_kit($item_id)) {
                            ...
                            echo json_encode(array(
                                'success' => false,
                                'message' => lang('batch_receivings_error')
                            ));
                            return;
                        }
        ...
        echo json_encode(array(
            'success' => true,
            'message' => lang('receivings_import_successfull')
        ));
    }
    ...
    function excel()
    {
        ...
        array_to_spreadsheet(array(
            $header_row
        ), 'batch_receiving_export.' . ($this->config->item('spreadsheet_format') == 'XLSX' ? 'xlsx' : 'csv'));
    }
    ...
    function save($receiving_id)
    {
        ...
        if ($this->Receiving->update($receiving_data, $receiving_id)) {
            echo json_encode(array(
                'success' => true,
                'message' => lang('receivings_successfully_updated')
            ));
        } else {
            echo json_encode(array(
                'success' => false,
                'message' => lang('receivings_unsuccessfully_updated')
            ));
        }
    }
    ...
    function _reload($data = array(), $is_ajax = true)
    {
        ...
        $data['cart'] = $this->receiving_lib->get_cart();
        $data['modes'] = array(
            'receive' => lang('receivings_receiving'),
            'return' => lang('receivings_return'),
            'purchase_order' => lang('receivings_purchase_order')
        );
        ...
        switch ($criteria) {
            ...
            case 'below_reorder_level_and_out_of_stock':
                ...
                $model->setParams(array(
                    ...
                    'export_excel' => 1,
                    'offset' => 0,
                    'inventory' => $criteria == 'below_reorder_level' ? 'all' : 'out_of_stock',
                    'reorder_only' => true,
    ...
    function add_payment()
    {
        ...
        if (($this->input->post('payment_type') == lang('common_store_account') && $this->receiving_lib->get_supplier() == - 1) || ($this->receiving_lib->get_mode() == 'store_account_payment' && $this->receiving_lib->get_supplier() == - 1)) {
