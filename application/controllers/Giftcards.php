...
class Giftcards extends Secure_area implements Idata_controller
{
    ...
    {
        $params = $this->session->userdata('giftcard_search_data') ? $this->session->userdata('giftcard_search_data') : array(
            'offset' => 0,
            'order_col' => 'giftcard_number',
            'order_dir' => 'asc',
            'search' => FALSE
        );
    ...
    function excel()
    {
        ...
        array_to_spreadsheet(array(
            $header_row
        ), 'giftcards_import.' . ($this->config->item('spreadsheet_format') == 'XLSX' ? 'xlsx' : 'csv'));
    }
    ...
    function excel_export()
    {
        ...
        if ($search) {
            $data = $this->Giftcard->search($search, $this->Giftcard->search_count_all($search), 0, $params['order_col'], $params['order_dir'])
                ->result_object();
        } else {
    ...
    function do_excel_import()
    {
            ...
            if (($handle = fopen($_FILES['file_path']['tmp_name'], "r")) !== FALSE) {
                ...
                for ($k = 2; $k <= $num_rows; $k ++) {
                    ...
                    if (! $this->Giftcard->save($giftcard_data, $giftcard_id ? $giftcard_id : FALSE)) {
                        echo json_encode(array(
                            'success' => false,
                            'message' => lang('giftcards_duplicate_giftcard')
                        ));
                        return;
                    }
                    ...
                    if ($old_giftcard_value != $giftcard_data['value']) {
                        $this->Giftcard->log_modification(array(
                            "number" => $giftcard_data['giftcard_number'],
                            "person" => $employee_info->first_name . " " . $employee_info->last_name,
                            "new_value" => $giftcard_data['value'],
                            'keyword' => $keyword,
                            'old_value' => $old_giftcard_value,
                            "type" => $giftcard_id ? 'update' : 'create'
                        ));
                    }
                }
            } else {
        ...
        echo json_encode(array(
            'success' => true,
            'message' => lang('giftcards_import_success')
        ));
    }
    ...
    function save($giftcard_id = -1)
    {
            ...
            } else // previous giftcard
            {
                if ($giftcard_data['value'] > $old_giftcard_value) {
                    ...
                    $this->Giftcard->log_modification(array(
                        ...
                        'old_value' => $old_giftcard_value,
                        "type" => "update",
                        "keyword" => lang('giftcards_removed')
                    ));
                }
        ...
        } else // failure
        {
            echo json_encode(array(
                'success' => false,
                'message' => lang('giftcards_error_adding_updating') . ' ' . $giftcard_data['giftcard_number'],
                'giftcard_id' => - 1
            ));
        }
    }
    ...
    function delete()
    {
        ...
        if ($this->Giftcard->delete_list($giftcards_to_delete)) {
            echo json_encode(array(
                'success' => true,
                'message' => lang('giftcards_successful_deleted') . ' ' . count($giftcards_to_delete) . ' ' . lang('giftcards_one_or_multiple')
            ));
        } else {
            echo json_encode(array(
                'success' => false,
                'message' => lang('giftcards_cannot_be_deleted')
            ));
        }
    }
    ...
    function generate_barcodes($giftcard_ids, $skip = 0)
    {
        ...
        foreach ($giftcard_ids as $giftcard_id) {
            ...
            $result[] = array(
                'name' => $giftcard_info->giftcard_number . ': ' . to_currency($giftcard_info->value, 10),
                'id' => $giftcard_info->giftcard_number
            );
        }
    ...
    function save_item($item_id = -1)
    {
        ...
        if ($this->Item->save($item_data, $item_id)) {
            ...
            if ($item_id == - 1) {
                echo json_encode(array(
                    'success' => true,
                    'message' => lang('common_successful_adding') . ' ' . $item_data['name'],
            ...
            } else // previous item
            {
                echo json_encode(array(
                    'success' => true,
                    'message' => lang('common_items_successful_updating') . ' ' . $item_data['name'],
                    'item_id' => $item_id,
                    'redirect' => $redirect,
                    'sale_or_receiving' => $sale_or_receiving
                ));
