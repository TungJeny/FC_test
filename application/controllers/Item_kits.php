...
class Item_kits extends Secure_area implements Idata_controller
{
    ...
    {
        $params = $this->session->userdata('item_kit_search_data') ? $this->session->userdata('item_kit_search_data') : array(
            'offset' => 0,
            'order_col' => 'item_kit_id',
            'order_dir' => 'desc',
            'search' => FALSE,
            'category_id' => FALSE,
            'fields' => 'all'
    ...
    function excel_export()
    {
        ...
        if ($search || $category_id) {
            $data = $this->Item_kit->search($search, $category_id, $this->Item_kit->search_count_all($search, $category_id, 10000, $params['fields']), 0, $params['order_col'], $params['order_dir'], $params['fields'])
                ->result_object();
        } else {
        ...
        $rows = array();
        $row = array(
            lang('common_item_number'),
            lang('common_product_id'),
            lang('item_kits_name'),
            lang('common_category'),
            lang('common_manufacturer'),
            lang('common_cost_price'),
            lang('common_unit_price'),
            lang('item_kits_tax_1_name'),
            lang('item_kits_tax_1_percent'),
            lang('item_kits_tax_2_name'),
            lang('item_kits_tax_2_percent'),
            lang('item_kits_tax_2_cummulative'),
    ...
    {
        echo json_encode(array(
            'duplicate' => $this->Item_kit->check_duplicate($this->input->post('term'))
        ));
    }
    ...
    function save($item_kit_id = -1)
    {
        ...
        } else // failure
        {
            echo json_encode(array(
                'success' => false,
                'message' => lang('item_kits_error_adding_updating') . ' ' . $item_kit_data['name'],
                'item_kit_id' => - 1
            ));
        }
    }
    ...
    function cleanup()
    {
        ...
        echo json_encode(array(
            'success' => true,
            'message' => lang('item_kits_cleanup_sucessful')
        ));
    }
