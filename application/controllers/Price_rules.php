...
class Price_rules extends Secure_area implements Idata_controller
{
    ...
    {
        $params = $this->session->userdata('price_rule_search_data') ? $this->session->userdata('price_rule_search_data') : array(
            'offset' => 0,
            'order_col' => 'name',
            'order_dir' => 'asc',
            'search' => FALSE
                    ...
                    switch ($this->input->get('w')) {
                        ...
                        case 'itemsCategory':
                            ...
                            foreach ($t as $k => $v) {
                                $tmp[$k] = array(
                                    'id' => $v['id'],
                                    'name' => $this->Category->get_full_path($v['id'])
                                );
                            }
                            die(json_encode($tmp));
                            break;
    ...
    function _get_pricerule_data($rule_id)
    {
        ...
        foreach ($rulecats as $key => $val) {
            if ($rule_info['type'] !== 'spend_x_get_discount') {
                $rule_cats[$key] = array(
                    'id' => $val['category_id'],
                    'name' => $this->Category->get_full_path($val['category_id'])
                );
            }
        }
    ...
    function delete()
    {
        ...
        if ($this->Price_rule->delete($rule_ids)) {
            echo json_encode(array(
                'success' => true,
                'message' => lang('price_rules_rule_deleted')
            ));
        } else {
            echo json_encode(array(
                'success' => false,
                'message' => lang('price_rules_cannot_be_deleted')
            ));
        }
    }
