...
class Login extends MY_Controller
{
    ...
    function index()
    {
        ...
        if (ENVIRONMENT == 'development' && $this->migration->need_to_migrate()) {
        ...
        if ($username && $this->_can_fast_switch_user($username)) {
            if (! $this->Employee->login_no_password($username)) {
                ...
                echo json_encode(array(
                    ...
                    'name' => $name,
                    'avatar' => $avatar,
                    'is_clocked_in_or_timeclock_disabled' => $is_clocked_in_or_timeclock_disabled
                ));
            }
        } elseif ($username) {
    ...
    {
        return $this->db->delete('employees_reset_password', array(
            'key' => $key
        ));
    }
