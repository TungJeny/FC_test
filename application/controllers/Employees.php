...
class Employees extends Person_controller
{
    ...
    {
        $params = $this->session->userdata('employee_search_data') ? $this->session->userdata('employee_search_data') : array(
            'offset' => 0,
            'order_col' => 'last_name',
            'order_dir' => 'asc',
            'search' => FALSE
    ...
    function sorting()
    {
        ...
        if ($search) {
            ...
            $config['total_rows'] = $this->Employee->count_all();
            $table_data = $this->Employee->get_all($per_page, $this->input->post('offset') ? $this->input->post('offset') : 0, $this->input->post('order_col') ? $this->input->post('order_col') : 'last_name', $this->input->post('order_dir') ? $this->input->post('order_dir') : 'asc', TRUE);
        }
        $config['base_url'] = site_url('employees/sorting');
    ...
    function excel_export()
    {
        ...
        if ($search) {
            $data = $this->Employee->search($search, $this->Employee->search_count_all($search), 0, $params['order_col'], $params['order_dir'])
                ->result_object();
        } else {
    ...
    function save($employee_id = -1)
    {
        ...
        if ($this->input->post('password') != '') {
            $employee_data = array(
                ...
                'is_shipper' => $this->input->post('is_shipper'),
                'is_importer' => $this->input->post('is_importer'),
        ...
        $employee_data = array_merge($employee_data, array(
        ...
        if ((is_on_demo_host()) && $employee_id == 1) {
            ...
            echo json_encode(array(
                'success' => false,
                'message' => lang('common_employees_error_updating_demo_admin'),
                'person_id' => - 1
            ));
        ...
        } else // failure
        {
            echo json_encode(array(
                'success' => false,
                'message' => lang('common_employees_error_adding_updating') . ' ' . $person_data['first_name'] . ' ' . $person_data['last_name'],
                'person_id' => - 1
            ));
        }
    }
    ...
    function delete()
    {
        ...
        if (in_array(1, $employees_to_delete)) {
            ...
            echo json_encode(array(
                'success' => false,
                'message' => lang('employees_cannot_delete_default_user')
            ));
        } elseif ($this->Employee->delete_list($employees_to_delete)) {
            echo json_encode(array(
                'success' => true,
                'message' => lang('employees_successful_deleted') . ' ' . count($employees_to_delete) . ' ' . lang('employees_one_or_multiple')
            ));
        } else {
            echo json_encode(array(
                'success' => false,
                'message' => lang('employees_cannot_be_deleted')
            ));
        }
    }
    ...
    function cleanup()
    {
        ...
        echo json_encode(array(
            'success' => true,
            'message' => lang('employees_cleanup_sucessful')
        ));
    }
