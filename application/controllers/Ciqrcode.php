...
require_once (APPPATH . 'libraries/qrcode/qrconst.php');
require_once (APPPATH . 'libraries/qrcode/qrtools.php');
require_once (APPPATH . "libraries/qrcode/qrspec.php");
require_once (APPPATH . "libraries/qrcode/qrimage.php");
require_once (APPPATH . "libraries/qrcode/qrinput.php");
require_once (APPPATH . "libraries/qrcode/qrbitstream.php");
require_once (APPPATH . "libraries/qrcode/qrsplit.php");
require_once (APPPATH . "libraries/qrcode/qrrscode.php");
require_once (APPPATH . "libraries/qrcode/qrmask.php");
require_once (APPPATH . "libraries/qrcode/qrencode.php");

class Ciqrcode extends MY_Controller
{
	var $cacheable = true;
	...
	public function index() {
		...
		//$params = array()
		$name = 'Tên Sản Phẩm: '.rawurldecode($this->input->get('text'));
		isset($name) ? $name : "";
        $ciqrcode = 'ID Sản Phẩm: '.rawurldecode($this->input->get('ciqrcode'));
        isset($ciqrcode) ? $ciqrcode : "";

        $website = $this->input->get('website') ? 'Địa Chỉ Web: '.$this->input->get('website') : '';
        $company = $this->input->get('company') ? 'Tên Công Ty: '.$this->input->get('company') : '';
        $lvt = $this->input->get('lvt') ? 'Lô Vật Tư: '.$this->input->get('lvt') : '';
        $ltp = $this->input->get('ltp') ? 'Lô Thành Phẩm: '.$this->input->get('ltp') : '';

        $params['data'] = '';     
        $params['data'] .= $company."\n";
        $params['data'] .= $website."\n";
        $params['data'] .= $ciqrcode."\n";
        $params['data'] .= $name."\n";
        $params['data'] .= $lvt."\n";
        $params['data'] .= $ltp."\n";
