								...
								<?php foreach($tiers as $tier) { ?>	
									<div class="form-group">
										...
										<?php echo form_input(array(
											...
											'size'=>'8',
											'value'=> $location_tier_prices[$location->location_id][$tier->id] !== FALSE ? ($location_tier_prices[$location->location_id][$tier->id]->unit_price != NULL ? to_currency_no_money($location_tier_prices[$location->location_id][$tier->id]->unit_price, 10) : to_quantity(($location_tier_prices[$location->location_id][$tier->id]->percent_off != NULL ) ? $location_tier_prices[$location->location_id][$tier->id]->percent_off : $location_tier_prices[$location->location_id][$tier->id]->cost_plus_percent )): '')
										);?>
										...
										echo form_dropdown('locations['.$location->location_id.'][tier_type]['.$tier->id.']', $tier_type_options, $selected_location_tier_type_option);?>
										</div>
									</div>
