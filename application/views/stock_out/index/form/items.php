<table class="table table-hover">
    ...
    <tr class="register-items-header">
        <th></th>
        <th class="item_name_heading"><?php echo lang('common_stock_out_item_name'); ?>    </th>
        <th class="sales_price"><?php echo lang('common_stock_out_cost'); ?>   </th>
        <th class="sales_quantity"><?php echo lang('common_quantity'); ?>   </th>
        <th><?php echo lang('common_stock_out_total'); ?></th>
    </tr>
    </thead>
    ...
    <?php foreach(array_reverse($cart, true) as $item_id => $item): ?>
        ...
        <tr class="register-item-details">
            <td class="text-center"> <?php echo anchor("stock_out/delete_item/$item_id",'<i class="icon ion-android-cancel"></i>', array('class' => 'delete-item')); ?> </td>
            <td>
            ...
            <td class="text-center">
                ...
                <a href="javascript:void(0)" id="price_<?php echo $item_id;?>" class="edit-on-click editable-click" data-keyboard-type="numpad" data-value="<?php echo H(to_currency_no_money($item['price'],10)); ?>" data-name="price" data-url="<?php echo site_url('stock_out/update_item/' . $item_id); ?>" data-title="<?php echo H(lang('common_price')); ?>"><?php echo to_currency($item['price'],10); ?></a>
                <?php else: ?>
                <a href="#" id="price_<?php echo $item_id; ?>" class="xeditable xeditable-price" data-validate-number="true" data-type="text" data-value="<?php echo H(to_currency_no_money($item['price'],10)); ?>" data-pk="1" data-name="price" data-url="<?php echo site_url('stock_out/update_item/' . $item_id); ?>" data-title="<?php echo H(lang('common_price')); ?>"><?php echo to_currency($item['price'],10); ?></a>
                <?php endif; ?>
                <?php else: ?>
            ...
            <td class="text-center">
                <?php if ($this->config->item('virtual_keyboard') == 'all' || ($this->agent->is_mobile() && $this->config->item('virtual_keyboard')  == 'mobile') ||  (!$this->agent->is_mobile() && $this->config->item('virtual_keyboard')  == 'desktop')): ?>
                <a href="javascript:void(0)" id="quantity_<?php echo $item_id; ?>" class="edit-on-click editable-click" data-keyboard-type="numpad" data-value="<?php echo H(to_quantity($item['quantity'])); ?>" data-name="quantity" data-url="<?php echo site_url('stock_out/update_item/'.$item_id); ?>" data-title="<?php echo lang('common_quantity') ?>"><?php echo to_quantity($item['quantity']); ?></a>
                <?php else: ?>
                <a href="#" id="quantity_<?php echo $item_id; ?>" class="xeditable" data-type="text"  data-validate-number="true" data-value="<?php echo H(to_quantity($item['quantity'])); ?>" data-pk="1" data-name="quantity" data-url="<?php echo site_url('stock_out/update_item/'.$item_id); ?>" data-title="<?php echo lang('common_quantity') ?>"><?php echo to_quantity($item['quantity']); ?></a>
                <?php endif; ?>
            </td>
