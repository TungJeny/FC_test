...
<div class="register-box pd-15 white-bg">
    <div class="customer-form">
        <!-- if the supplier is not set , show supplier adding form -->
        <?php echo form_open("stock_out/select_item_tp",array('id' => 'frm-select-itemTp', 'autocomplete' => 'off')); ?>
        <div class="input-group">
            <span class="input-group-btn">
                <a class = "btn btn-primary">Thành phẩm</a>
            </span>
            <input type="text" id="item-tp" name="item-tp" class="add-customer-input form-control validation" required data-value = "<?php echo $this->session->userdata('stock_out_package_item_tp') <=0? '': $this->session->userdata('stock_out_package_item_tp')?>" error-message = "item-tp-error" data-title="Thành phẩm" placeholder="Nhập tên thành phẩm..." autocomplete="off"/>
        </div>
        <span id = "item-tp-error"></span>
        <?php echo form_close(); ?>
        <?php if (!empty($item_tp)): ?>
        <div class="supplier mt-15">
            <div class="avatar pull-left">
            </div>
            <a tabindex="-1" class="bold">
                <?php echo character_limiter(H($item_tp_name), 60); ?>
            </a>
            <div class="cl"></div>
        </div>
        <?php endif; ?>
...
<div class="register-box pd-15 white-bg mt-15">
    <div class="employee-form">
        ...
        <div class="input-group">
            <span class="input-group-btn">
               	<a id = "new-employee" href ="javascript:void(0)" class = "btn btn-primary">
            		<i class='ion-plus'></i>
            	</a>
            </span>
...
<script type="text/javascript">
    ...
    $( "#item-tp" ).autocomplete({
        source: '<?php echo site_url("stock_out/search_item_tp");?>',
        delay: 150,
        autoFocus: false,
        minLength: 0,
        select: function(event, ui)
        {
            $.post('<?php echo site_url("stock_out/select_item_tp");?>', {item_tp: ui.item.value }, function(response) {
                $("#ajax-result").html(response);
            });
        },
    }).data("ui-autocomplete")._renderItem = function (ul, item) {
