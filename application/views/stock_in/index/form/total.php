<div class="register-box pd-15 white-bg">
	<div class="customer-form">
		<a href="stock_in/cancel_stock_in" class="btn btn-danger"
			style="border: none;" id="stock-in-cancel">Hủy đơn hàng</a>
	</div>
</div>
<div class="register-box pd-15 white-bg">
	<div class="customer-form">
        ...
        <div class="input-group">
			<span class="input-group-btn">
                <?php echo anchor("suppliers/view/-1/3","<i class='ion-plus'></i>", array('class' => 'btn btn-primary', 'title' => lang('stock_in_new_supplier'), 'id' => 'new-customer')); ?>
            </span> <input type="text" id="supplier"
				data-value="<?php echo  $this->session->userdata('supplier')<=0 ? '' : $this->session->userdata('supplier');?>"
				name="supplier" required
				class="add-customer-input form-control validation"
				error-message="supplier-error" data-title="Nhà cung cấp"
        ...
        <div class="supplier mt-15">
			...
			<a tabindex="-1"
				href="<?php echo site_url("suppliers/view/$supplier_id/3"); ?>"
				class="bold">
                <?php echo character_limiter(H($supplier), 30); ?>
                <?php if ($this->config->item('suppliers_store_accounts') && isset($supplier_balance)): ?>
                <span
				class="<?php echo $has_balance ? 'text-danger' : 'text-success'; ?> balance">(<?php echo to_currency($supplier_balance); ?>)</span>
                <?php endif; ?>
            </a>
...
<div class="register-box pd-15 white-bg mt-15">
	<div class="employee-form">
        ...
        <div class="input-group">
			<span class="input-group-btn">
                <?php echo anchor("employees/view/-1/3","<i class='ion-plus'></i>", array('class' => 'btn btn-primary', 'title' => lang('stock_in_new_employee'), 'id' => 'new-employee')); ?>
            </span> <input type="text" id="employee" required
				data-value="<?php echo  $this->session->userdata('employee') <= 0? '' : $this->session->userdata('employee');?>"
				name="employee" class="add-employee-input form-control validation"
				error-message="employee-error" data-title="Nhà cung cấp"
        ...
        <?php if (!empty($employee)): ?>
			...
			<a tabindex="-1"
				href="<?php echo site_url("employees/view/$employee_id/3"); ?>"
				class="bold">
...
<div class="register-box pd-15 white-bg mt-15">
    ...
    <div class="mt-15">
		<div class="pull-left">
			<div class="mt-5">
				<span class="key"><?php echo lang('common_total'); ?>: </span> <span
					class="value font-18"><?php echo to_currency($total); ?></span>
			</div>
		</div>
...
<script type="text/javascript">
    $( "#supplier" ).autocomplete({
        source: '<?php echo site_url("stock_in/search_supplier");?>',
        delay: 150,
        autoFocus: false,
        minLength: 0,
        select: function(event, ui)
        {
            $.post('<?php echo site_url("stock_in/select_supplier");?>', {supplier: ui.item.value }, function(response) {
