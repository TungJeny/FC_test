...
var app = new Vue({
    ...
    methods: {
        load_materials_matrix: function() {
            const that = this;
            that.loading = true;
            axios.get(SITE_URL + 'materials_plan/matrix_detail/' + CONST.MRP_CATEGORY_ID_PHOI).then(function(response){
                ...
                setTimeout(function(){
                    ...
                    $('#detail #contenthead-' + mode).scrollLeft((month * 150));
                    $('#detail #contentbody-' + mode).scrollLeft((month * 150));
                }, 10);
        ...
        fnSave: function() {
            ...
            if (active_tab == '#detail') {
                axios.post(SITE_URL + 'materials_plan/save', convert_2_formdata({
                        ...
                        category_id: CONST.MRP_CATEGORY_ID_PHOI
        ...
        showSummaryOfMonth: function() {
            const that = this;
            that.loading = true;
            axios.get(SITE_URL + 'materials_plan/matrix_summary/' + CONST.MRP_CATEGORY_ID_PHOI).then(function(response){
                that.loading = false;
