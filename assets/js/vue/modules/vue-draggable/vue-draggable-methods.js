  ...
  registerListeners(el) {
    ...
    el.addEventListener('dragend', (e) => {
      ...
      if (this.selections.droptarget) {
        ...
        for(var ulId in this.defaultOptions.itemList) {
        	let _items = [];
            let childItems = document.getElementById(ulId).getElementsByTagName(this.defaultOptions.draggableSelector);
            for(let i = 0; i < childItems.length ; i++) {
            	_items.push({'id': childItems[i].getAttribute('data-id'), 'label': childItems[i].textContent});
            }
            this.selections[ulId + 'Items'] = _items;
        }
        
        let targetItems = [];
        let childTargetItems = document.getElementById('target').getElementsByTagName(this.defaultOptions.draggableSelector);
        for(let i = 0; i < childTargetItems.length ; i++) {
        	targetItems.push({'id': childTargetItems[i].getAttribute('data-id'), 'label': childTargetItems[i].textContent});
        }
        this.selections.targetItems = targetItems;
        
        let ownerItems = [];
        let childOwnerItems = document.getElementById('owner').getElementsByTagName(this.defaultOptions.draggableSelector);
        for(let i = 0; i < childOwnerItems.length ; i++) {
        	ownerItems.push({'id': childOwnerItems[i].getAttribute('data-id'), 'label': childOwnerItems[i].textContent});
        }
        this.selections.ownerItems = ownerItems;
